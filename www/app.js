...
function recalculateFoodPurchaseCosts(p){
  ...
  const total=Math.round(Number(p.price)||0),each=Math.floor(total/portions.length),remainder=total-each*portions.length;
  portions.forEach((x,index)=>x.cost=each+(index<remainder?1:0));
}
function foodQuickForm(){return `<div class="card food-entry"><div class="card-head food-entry-head"><h3>식품 구매 기록</h3><button class="btn primary" id="saveFoodEntry">+ 구매 기록 저장</button></div><div class="food-form food-purchase-form"><label>구매일<input id="foodDate" type="date" value="${iso(new Date())}"></label><label>식품명<input id="foodTitle" placeholder="예: 닭갈비, 요구르트, 계란"></label><label>구매 가격<input id="foodAmount" inputmode="numeric" placeholder="예: 12000"></label><label>구매 수량<input id="foodQuantity" inputmode="decimal" value="1" placeholder="묶음·개수"></label><label class="food-memo">메모<input id="foodMemo" placeholder="용량, 브랜드 등 선택 입력"></label></div></div>`}
...
function foodInventory(){
  ...
  return `<div class="card section-gap"><div class="card-head"><h3>보유 식품과 먹기 기록</h3><b>${list.filter(x=>!foodIsFinished(x)).length}개 보유</b></div>${list.length?`<div class="food-inventory-grid">${list.map(p=>{const events=foodEvents(p).sort((a,b)=>a.date.localeCompare(b.date)||String(a.createdAt).localeCompare(String(b.createdAt))),used=foodConsumed(p),remaining=foodRemaining(p),finished=foodIsFinished(p),fixed=p.trackingMode==='fixed',pct=finished?100:fixed?Math.min(100,used/p.servings*100):0;return `<article class="food-purchase-card ${finished?'finished':''}"><div class="food-purchase-head"><div><span>${esc(p.purchaseDate)}</span><h4>${esc(p.name)}</h4></div><strong>${money(p.price)}</strong></div><div class="food-purchase-meta"><span>구매 ${p.quantity}개</span><span>${used.toFixed(2).replace(/\.00$/,'')}회 먹음</span>${finished&&used?`<span>회당 ${money(p.price/used)}</span>`:''}</div><div class="food-progress"><i style="width:${pct}%"></i></div><div class="food-remain"><b>${finished?'다 먹음':fixed?`${remaining.toFixed(2).replace(/\.00$/,'')}회분 남음`:`${used.toFixed(2).replace(/\.00$/,'')}회 먹은 상태`}</b></div>${!finished?`<div class="food-consume-actions"><button data-food-consume="${p.id}">1회 먹음</button><button class="primary-mini" data-food-finish="${p.id}">다 먹음</button></div>`:''}<div class="food-card-history"><b>먹은 기록</b>${events.length?`<div>${events.map(x=>`<button data-food-consumption-edit="${x.id}"><span>${esc(x.date)}</span><strong>${x.type==='finish'?'다 먹음':'1회 먹음'}</strong></button>`).join('')}</div>`:`<span>아직 먹은 기록이 없습니다.</span>`}</div><div class="food-card-foot"><span>${esc(p.memo)||'메모 없음'}</span><button data-food-edit="${p.id}">수정</button><button class="danger-text" data-food-delete="${p.id}">삭제</button></div></article>`}).join('')}</div>`:empty('위에서 산 음식과 가격을 먼저 기록해 보세요.')}</div>`
}
...
function foodStats(){
  const list=S.foodConsumptions.filter(x=>x.date.startsWith(selectedMonth)),eaten=list.filter(x=>x.type!=='finish'||Number(x.cost)>0),total=eaten.reduce((a,x)=>a+(Number(x.cost)||0),0),[y,m]=selectedMonth.split('-').map(Number),days=new Date(y,m,0).getDate(),stats=foodItemStats();
  return `<div class="grid cols4 section-gap">${metric('🍴','soft-red','이번 달 먹은 식비',money(total),`${eaten.length}회 먹음`)}${metric('日','soft-green','1일 평균',money(total/days),`${days}일 기준`)}${metric('週','soft-blue','주 평균',money(total/(days/7)),'먹은 비용 기준')}${metric('▦','soft-orange','먹은 식품',`${stats.length}종`,'식비 기록 기준')}</div><div class="card section-gap"><div class="card-head"><h3>항목별 소비 통계</h3></div>${stats.length?`<div class="table-wrap"><table class="table food-stats-table"><thead><tr><th>식품</th><th>먹은 횟수</th><th>먹은 분량</th><th>월 식비</th><th>회당 평균</th><th>최근 섭취</th></tr></thead><tbody>${stats.map(x=>`<tr><td><b>${esc(x.name)}</b></td><td>${x.count}회</td><td>${x.servings.toFixed(2).replace(/\.00$/,'')}회분</td><td class="red"><b>${money(x.cost)}</b></td><td>${money(x.cost/x.count)}</td><td>${x.last}</td></tr>`).join('')}</tbody></table></div>`:empty('먹은 기록을 추가하면 식품별 통계가 표시됩니다.')}</div><div class="card section-gap"><div class="card-head"><h3>먹은 식비 추이</h3><div class="tabs compact">${['일','주','월'].map(x=>`<button data-food-mode="${x}" class="${foodMode===x?'active':''}">${x}</button>`).join('')}</div></div>${barChart(foodTrend(),'#ff585d')}</div>`
}
function foodPrices(){
...
function food(){return header('식비 관리')+foodQuickForm()+foodTabs()+(foodTab==='history'?foodHistory():foodTab==='stats'?foodStats():foodTab==='prices'?foodPrices():foodInventory())}
...
function finishFoodModal(id){const p=foodPurchase(id);if(!p||foodIsFinished(p))return;confirmDialog({title:'다 먹음',message:`${p.name}을 다 먹은 것으로 처리할까요?`,confirmText:'확인',onConfirm:()=>{const date=iso(new Date()),servings=p.trackingMode==='fixed'?foodRemaining(p):0;S.foodConsumptions.push({id:uid(),purchaseId:p.id,type:'finish',date,servings,cost:foodCostFor(p,servings),memo:'',createdAt:new Date().toISOString()});recalculateFoodPurchaseCosts(p);selectedMonth=date.slice(0,7);render();toast(`${p.name}을 다 먹은 것으로 기록했습니다.`)}})}

...
function normalizeFoodData(){
  ...
  S.foodPurchases=S.foodPurchases.map(x=>{const trackingMode=x.trackingMode==='fixed'?'fixed':'until-finished',name=String(x.name||x.category||'이름 없음').trim()||'이름 없음',packageUnit=['g','kg','ml','L','개'].includes(x.packageUnit)?x.packageUnit:'개';return {...x,id:String(x.id||uid()),name,category:String(x.category||name).trim()||name,variant:String(x.variant||'').trim(),purchaseDate:/^\d{4}-\d{2}-\d{2}$/.test(x.purchaseDate||'')?x.purchaseDate:iso(today),store:String(x.store||'').trim(),price:Math.max(0,Number(x.price)||0),quantity:Math.max(.01,Number(x.quantity)||1),packageAmount:Math.max(.01,Number(x.packageAmount)||1),packageUnit,trackingMode,servings:trackingMode==='fixed'?Math.max(.01,Number(x.servings)||1):null,memo:String(x.memo||'').trim(),createdAt:x.createdAt||new Date().toISOString()}});
  const ids=new Set(S.foodPurchases.map(x=>x.id));
  S.foodConsumptions=S.foodConsumptions.filter(x=>ids.has(String(x.purchaseId))).map(x=>{const type=x.type==='finish'?'finish':'portion',unit=['g','kg','ml','L','개','회'].includes(x.unit)?x.unit:'회';return {...x,id:String(x.id||uid()),purchaseId:String(x.purchaseId),type,date:/^\d{4}-\d{2}-\d{2}$/.test(x.date||'')?x.date:iso(today),servings:type==='finish'?Math.max(0,Number(x.servings)||0):Math.max(.01,Number(x.servings)||1),amount:type==='finish'?0:Math.max(.01,Number(x.amount)||Number(x.servings)||1),unit,cost:Math.max(0,Number(x.cost)||0),memo:String(x.memo||'').trim(),createdAt:x.createdAt||new Date().toISOString()}}).filter(x=>x.type==='finish'||x.servings>0);
...
const FOOD_UNITS=['g','kg','ml','L','개'];
const foodEvents=p=>S.foodConsumptions.filter(x=>x.purchaseId===p.id);
const foodPortions=p=>foodEvents(p).filter(x=>x.type!=='finish');
const foodConsumed=p=>p.trackingMode==='fixed'?foodPortions(p).reduce((a,x)=>a+Number(x.servings||0),0):foodPortions(p).length;
...
const foodPackageMeasure=p=>foodMeasure((Number(p.packageAmount)||1)*(Number(p.quantity)||1),p.packageUnit||'개');
...
function foodCostFor(p,servings){return p.trackingMode==='fixed'?Number(p.price)*(Number(servings)||0)/Math.max(Number(p.servings)||1,.01):0}
function recalculateFoodPurchaseCosts(p){
  if(!p)return;
  if(p.trackingMode==='fixed'){foodPortions(p).forEach(x=>x.cost=foodCostFor(p,x.servings));return}
  const events=foodEvents(p),portions=foodPortions(p),finish=foodFinishEvent(p),pack=foodPackageMeasure(p),total=Math.round(Number(p.price)||0);
  events.forEach(x=>x.cost=0);
  let allocated=0;
  if(pack.amount&&pack.dimension!=='event'){
    portions.forEach(x=>{const m=foodEventMeasure(x);if(m.dimension===pack.dimension){x.cost=Math.max(0,Math.min(total-allocated,Math.round(total*Math.min(m.amount,pack.amount)/pack.amount)));allocated+=x.cost}});
  }else if(finish&&portions.length){
    const each=Math.floor(total/portions.length),remainder=total-each*portions.length;
    portions.forEach((x,index)=>x.cost=each+(index<remainder?1:0));allocated=total;
  }
  if(finish)finish.cost=Math.max(0,total-Math.min(total,allocated));
}
...
function foodQuickForm(){const last=foodLastPurchase(),category=last?foodCategory(last):'',variant=last?.variant||'',store=last?.store||'',amount=last?.packageAmount||1,unit=last?.packageUnit||'개',quantity=last?.quantity||1;return `<div class="card food-entry"><div class="card-head food-entry-head"><h3>식품 구매 기록</h3><button class="btn primary" id="saveFoodEntry">+ 구매 기록 저장</button></div><div class="food-form food-purchase-form"><label>구매일<input id="foodDate" type="date" value="${iso(new Date())}"></label><label>식품 종류<input id="foodTitle" value="${esc(category)}" placeholder="예: 우유, 닭갈비, 브로콜리"></label><label>상품·브랜드<input id="foodVariant" value="${esc(variant)}" placeholder="예: 서울우유 2L"></label><label>마트·구매처<input id="foodStore" value="${esc(store)}" placeholder="예: 이마트, 쿠팡"></label><label>구매 가격<input id="foodAmount" inputmode="numeric" placeholder="예: 12000"></label><label>포장량<input id="foodPackageAmount" inputmode="decimal" value="${foodNumber(amount)}"></label><label>단위<select id="foodPackageUnit">${foodUnitOptions(unit)}</select></label><label>구매 수량<input id="foodQuantity" inputmode="decimal" value="${foodNumber(quantity)}"></label><label class="food-memo">메모<input id="foodMemo" placeholder="선택 입력"></label></div></div>`}
...
function foodStats(){
  const list=S.foodConsumptions.filter(x=>x.date.startsWith(selectedMonth)&&foodMatches(foodPurchase(x.purchaseId)||{})),eaten=list.filter(x=>x.type!=='finish'),total=list.reduce((a,x)=>a+(Number(x.cost)||0),0),[y,m]=selectedMonth.split('-').map(Number),days=new Date(y,m,0).getDate(),stats=foodItemStats();
...
function saveFoodEntry(){const name=$('#foodTitle').value.trim(),variant=$('#foodVariant').value.trim(),price=Math.abs(num($('#foodAmount').value)),packageAmount=Math.max(.01,decimalNum($('#foodPackageAmount').value)),packageUnit=$('#foodPackageUnit').value,quantity=Math.max(.01,decimalNum($('#foodQuantity').value));if(!name||!price)return toast('식품 종류와 구매 가격을 입력하세요.');const purchaseDate=$('#foodDate').value||iso(today);S.foodPurchases.push({id:uid(),name,category:name,variant,purchaseDate,store:$('#foodStore').value.trim(),price,quantity,packageAmount,packageUnit,trackingMode:'until-finished',servings:null,memo:$('#foodMemo').value.trim(),createdAt:new Date().toISOString()});selectedMonth=purchaseDate.slice(0,7);foodCategoryFilter=name;foodTab='inventory';render();toast(`${foodPurchaseTitle(S.foodPurchases.at(-1))} 구매 기록을 저장했습니다.`)}
function consumeFood(id,date,amount,unit,memo=''){const p=foodPurchase(id);if(!p||foodIsFinished(p))return toast('이미 다 먹은 식품입니다.');amount=Math.max(.01,Number(amount)||0);const pack=foodPackageMeasure(p),eaten=foodMeasuredConsumed(p),next=foodMeasure(amount,unit);if(next.dimension===pack.dimension&&eaten+next.amount>pack.amount+.0001)return toast(`남은 계산량 ${foodAmountLabel(Math.max(0,pack.amount-eaten),pack.unit)} 이하로 입력하세요.`);S.foodConsumptions.push({id:uid(),purchaseId:p.id,type:'portion',date,servings:1,amount,unit,cost:0,memo,createdAt:new Date().toISOString()});recalculateFoodPurchaseCosts(p);selectedMonth=date.slice(0,7);render();toast(`${foodPurchaseTitle(p)} ${foodAmountLabel(amount,unit)} 먹은 기록을 저장했습니다.`)}
function foodConsumeModal(id){const p=foodPurchase(id);if(!p||foodIsFinished(p))return;const previous=[...foodPortions(p)].sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)))[0],unit=previous?.unit&&previous.unit!=='회'?previous.unit:p.packageUnit||'개',amount=previous?.amount||(unit==='개'?1:'');$('#submodal').innerHTML=`<div class="modal"><div class="modalbox"><h2>${esc(foodPurchaseTitle(p))} 1회 먹음</h2><div class="modal-grid"><label>먹은 날짜<input id="foodConsumeDate" type="date" value="${iso(today)}"></label><label>먹은 양<input id="foodConsumeAmount" inputmode="decimal" value="${amount}"></label><label>단위<select id="foodConsumeUnit">${foodUnitOptions(unit)}</select></label><label class="wide">메모<input id="foodConsumeMemo" placeholder="선택 입력"></label></div><div class="modal-actions"><button class="pill" id="cancelFoodConsume">취소</button><button class="btn primary" id="confirmFoodConsume">기록</button></div></div></div>`;$('#cancelFoodConsume').onclick=closeSubmodal;$('#confirmFoodConsume').onclick=()=>{const date=$('#foodConsumeDate').value,amount=decimalNum($('#foodConsumeAmount').value),unit=$('#foodConsumeUnit').value,memo=$('#foodConsumeMemo').value.trim();if(!/^\d{4}-\d{2}-\d{2}$/.test(date))return toast('먹은 날짜를 선택하세요.');if(amount<=0)return toast('먹은 양을 입력하세요.');closeSubmodal();consumeFood(id,date,amount,unit,memo)}}
function finishFoodModal(id){const p=foodPurchase(id);if(!p||foodIsFinished(p))return;confirmDialog({title:'다 먹음',message:`${foodPurchaseTitle(p)}을 다 먹은 것으로 처리할까요?`,confirmText:'확인',onConfirm:()=>{const date=iso(new Date()),servings=p.trackingMode==='fixed'?foodRemaining(p):0;S.foodConsumptions.push({id:uid(),purchaseId:p.id,type:'finish',date,servings,amount:0,unit:p.packageUnit||'개',cost:0,memo:'',createdAt:new Date().toISOString()});recalculateFoodPurchaseCosts(p);selectedMonth=date.slice(0,7);render();toast(`${foodPurchaseTitle(p)}을 다 먹은 것으로 기록했습니다.`)}})}
function foodConsumptionEditModal(id){const x=S.foodConsumptions.find(v=>v.id===id),p=x&&foodPurchase(x.purchaseId);if(!x||!p)return;const portion=x.type!=='finish';$('#submodal').innerHTML=`<div class="modal"><div class="modalbox"><h2>${esc(foodPurchaseTitle(p))} ${portion?'먹은 기록':'다 먹음'} 수정</h2><div class="modal-grid"><label>날짜<input id="editFoodConsumptionDate" type="date" value="${x.date}"></label>${portion?`<label>먹은 양<input id="editFoodConsumptionAmount" inputmode="decimal" value="${x.amount||x.servings}"></label><label>단위<select id="editFoodConsumptionUnit">${foodUnitOptions(x.unit||'개')}</select></label>`:''}<label class="wide">메모<input id="editFoodConsumptionMemo" value="${esc(x.memo)}"></label></div><div class="modal-actions"><button class="pill" id="cancelFoodConsumptionEdit">취소</button><button class="btn primary" id="confirmFoodConsumptionEdit">저장</button></div></div></div>`;$('#cancelFoodConsumptionEdit').onclick=closeSubmodal;$('#confirmFoodConsumptionEdit').onclick=()=>{const date=$('#editFoodConsumptionDate').value;if(!/^\d{4}-\d{2}-\d{2}$/.test(date))return toast('날짜를 선택하세요.');if(portion){const amount=decimalNum($('#editFoodConsumptionAmount').value),unit=$('#editFoodConsumptionUnit').value,pack=foodPackageMeasure(p),next=foodMeasure(amount,unit),other=foodMeasuredConsumed(p,x.id);if(amount<=0)return toast('먹은 양을 입력하세요.');if(next.dimension===pack.dimension&&other+next.amount>pack.amount+.0001)return toast(`다른 기록을 제외한 남은 계산량 ${foodAmountLabel(Math.max(0,pack.amount-other),pack.unit)} 이하로 입력하세요.`);x.amount=amount;x.unit=unit;x.servings=1}x.date=date;x.memo=$('#editFoodConsumptionMemo').value.trim();recalculateFoodPurchaseCosts(p);selectedMonth=date.slice(0,7);closeSubmodal();render();toast('먹은 기록을 수정했습니다.')}}
...
function foodEditModal(id){const p=foodPurchase(id);if(!p)return;$('#submodal').innerHTML=`<div class="modal"><div class="modalbox"><h2>식품 구매 기록 수정</h2><div class="modal-grid"><label>구매일<input id="editFoodDate" type="date" value="${p.purchaseDate}"></label><label>식품 종류<input id="editFoodName" value="${esc(foodCategory(p))}"></label><label>상품·브랜드<input id="editFoodVariant" value="${esc(p.variant)}"></label><label>마트·구매처<input id="editFoodStore" value="${esc(p.store)}"></label><label>구매 가격<input id="editFoodPrice" inputmode="numeric" value="${p.price}"></label><label>포장량<input id="editFoodPackageAmount" inputmode="decimal" value="${p.packageAmount}"></label><label>단위<select id="editFoodPackageUnit">${foodUnitOptions(p.packageUnit)}</select></label><label>구매 수량<input id="editFoodQuantity" inputmode="decimal" value="${p.quantity}"></label><label class="wide">메모<input id="editFoodMemo" value="${esc(p.memo)}"></label></div><div class="modal-actions"><button class="pill" id="cancelFoodEdit">취소</button><button class="btn primary" id="confirmFoodEdit">저장</button></div></div></div>`;$('#cancelFoodEdit').onclick=closeSubmodal;$('#confirmFoodEdit').onclick=()=>{const name=$('#editFoodName').value.trim(),price=Math.abs(num($('#editFoodPrice').value)),packageAmount=Math.max(.01,decimalNum($('#editFoodPackageAmount').value));if(!name||!price)return toast('식품 종류와 가격을 입력하세요.');Object.assign(p,{purchaseDate:$('#editFoodDate').value,name,category:name,variant:$('#editFoodVariant').value.trim(),store:$('#editFoodStore').value.trim(),price,packageAmount,packageUnit:$('#editFoodPackageUnit').value,quantity:Math.max(.01,decimalNum($('#editFoodQuantity').value)),memo:$('#editFoodMemo').value.trim()});recalculateFoodPurchaseCosts(p);foodCategoryFilter=name;closeSubmodal();render();toast('구매 기록을 수정했습니다.')}}

...
const CATEGORY_ICON_SUGGESTIONS={
  '식재료':'🛒','외식/배달':'🍽️','간식/음료':'🥤','술/유흥':'🍺',
  '대중교통':'🚌','택시':'🚕','대여':'🔑','숙박':'🏨',
  '문화생활':'🎭','운동':'🏋️','병원/약국':'🏥','교육':'📚','놀러감':'🧳',
  '가구':'🪑','주방용품':'🍳','생활용품':'🧻','컴퓨터 제품':'💻','쇼핑':'🛍️','다이소':'🧺',
  '옷':'👕','머리':'💇','화장품':'💄','시술':'✨',
  '정기 구독':'🔄','월세':'🏠','통신비':'📱','현질':'🎮','스팀':'🕹️',
  '적금':'🏦','청약':'🏘️','주식':'📈','급여':'💰','기타 수입':'➕','용돈':'🎁','환급':'↩️','계좌 이체':'🔁'
};
const suggestCategoryIcon=name=>CATEGORY_ICON_SUGGESTIONS[String(name||'').trim()]||'•';
function normalizeCategoryIcons(){
  if(!Array.isArray(S.categories))S.categories=[];
  S.categories.forEach(x=>{if(!String(x.icon||'').trim()||/^[•·.]+$/.test(String(x.icon).trim()))x.icon=suggestCategoryIcon(x.item)});
}
...
function sample(){
  ...
  return {
    ...
    foodPurchases:[],
    foodConsumptions:[],
    settings:{dark:false,theme:'light',fontSize:'normal',autoSave:true,googleAutoSync:false,currency:'KRW',dateFormat:'YYYY-MM-DD',periodMode:'rolling'}
  };
}

let S=sample(),desktopInfo=null,googleInfo={connected:false},googleBackups=[],googleBackupPage=0,appReady=false,hadLocalData=false;
let page='dashboard', selectedMonth=monthKey(today), periodMode='rolling', txFilter='전체', lastTransactionDate=iso(today), lastTransactionDateAt=0, foodMode='일', foodTab='inventory', foodCategoryFilter='전체', foodEntryExpanded=false, analysisMode='overview', analysisArea='지출', analysisSection='식비', analysisItem='외식/배달', excludeRent=false;
...
const GOOGLE_BACKUP_PAGE_SIZE=7;
...
async function loadGoogleBackups({rerender=true}={}){
  const bridge=googleBridge();
  if(!googleInfo.connected||!bridge?.listBackups){googleBackups=[];googleBackupPage=0;if(rerender)render();return false}
  try{const result=await bridge.listBackups();googleBackups=(Array.isArray(result?.files)?result.files:[]).sort((a,b)=>backupTime(b)-backupTime(a));googleBackupPage=Math.min(googleBackupPage,Math.max(0,Math.ceil(googleBackups.length/GOOGLE_BACKUP_PAGE_SIZE)-1));if(result?.lastBackupTime)googleInfo.lastBackupTime=result.lastBackupTime;if(rerender)render();return true}
...
function calendarMonthAgo(value){const d=new Date(value),day=d.getDate();d.setDate(1);d.setMonth(d.getMonth()-1);d.setDate(Math.min(day,new Date(d.getFullYear(),d.getMonth()+1,0).getDate()));return d}
function periodBounds(){
  if(periodMode==='month'){const [y,m]=selectedMonth.split('-').map(Number);return {start:`${selectedMonth}-01`,end:iso(new Date(y,m,0))}}
  const end=new Date(),start=calendarMonthAgo(end);return {start:iso(start),end:iso(end)}
}
const dateInPeriod=date=>{const {start,end}=periodBounds();return /^\d{4}-\d{2}-\d{2}$/.test(date||'')&&date>=start&&date<=end};
const periodDates=()=>{const {start,end}=periodBounds(),out=[],d=new Date(`${start}T00:00:00`),last=new Date(`${end}T00:00:00`);while(d<=last){out.push(iso(d));d.setDate(d.getDate()+1)}return out};
const periodDayCount=()=>Math.max(1,periodDates().length);
const periodTitle=()=>periodMode==='rolling'?'최근 1개월':'선택 월';
const periodRangeLabel=()=>{const {start,end}=periodBounds();return periodMode==='rolling'?`${Number(start.slice(5,7))}.${Number(start.slice(8))} ~ ${Number(end.slice(5,7))}.${Number(end.slice(8))}`:`${Number(selectedMonth.slice(5))}월`};
function applyPeriodPreference(){S.settings=S.settings||sample().settings;periodMode=S.settings.periodMode==='month'?'month':'rolling';S.settings.periodMode=periodMode}
const FONT_SIZE_OPTIONS=['normal','large','xlarge'];
function applyFontPreference(){S.settings=S.settings||sample().settings;const value=FONT_SIZE_OPTIONS.includes(S.settings.fontSize)?S.settings.fontSize:'normal';S.settings.fontSize=value;document.documentElement.dataset.fontSize=value}
const monthTx=()=>S.transactions.filter(x=>dateInPeriod(x.date));
const calendarTx=()=>S.transactions.filter(x=>periodMode==='rolling'?dateInPeriod(x.date):x.date.startsWith(selectedMonth));
...
const cat=(a,s,i)=>S.categories.find(x=>(a==null||x.area===a)&&(s==null||x.section===s)&&x.item===i)||{icon:suggestCategoryIcon(i),color:'#8795aa'};
...
const rentFilterLabel=()=>excludeRent?'월세 제외':'월세 포함';
...
function normalizeTransactions(){normalizeCategoryIcons();S.transactions.forEach(x=>{if(x.area==='수입'||x.type==='수입'){x.type='수입';x.area='수입';x.amount=Math.abs(Number(x.amount)||0)}else if(x.area==='지출'||x.type==='지출'){x.type='지출';x.area='지출';x.amount=-Math.abs(Number(x.amount)||0)}else if(x.area==='이체'||x.type==='이체'){x.type='이체';x.area='이체'}})}
function normalizeFoodData(){
  ...
  S.foodPurchases=S.foodPurchases.map(x=>{const wasFixed=x.trackingMode==='fixed',name=String(x.name||x.category||'이름 없음').trim()||'이름 없음',source=x.source==='existing'?'existing':'purchase',legacyUnit=['g','kg','ml','L','개','회'].includes(x.packageUnit)?x.packageUnit:'개',legacyAmount=Math.max(.01,Number(x.packageAmount)||1),legacyQuantity=Math.max(.01,Number(x.quantity)||1),totalUnit=['g','kg','ml','L','개','회'].includes(x.totalUnit)?x.totalUnit:(wasFixed?'회':legacyUnit),totalAmount=Math.max(.01,Number(x.totalAmount)||(wasFixed?Number(x.servings)||1:legacyAmount*legacyQuantity)),finishedDate=/^\d{4}-\d{2}-\d{2}$/.test(x.finishedDate||'')?x.finishedDate:'',priceKnown=x.priceKnown===false?false:true;return {...x,id:String(x.id||uid()),name,category:String(x.category||name).trim()||name,variant:String(x.variant||'').trim(),source,priceKnown,purchaseDate:source==='existing'?'':(/^\d{4}-\d{2}-\d{2}$/.test(x.purchaseDate||'')?x.purchaseDate:iso(today)),store:String(x.store||'').trim(),price:priceKnown?Math.max(0,Number(x.price)||0):0,totalAmount,totalUnit,packageAmount:totalAmount,packageUnit:totalUnit,quantity:1,trackingMode:'free',servings:null,finishedDate,memo:String(x.memo||'').trim(),createdAt:x.createdAt||new Date().toISOString()}});
  const ids=new Set(S.foodPurchases.map(x=>x.id));
  S.foodConsumptions=S.foodConsumptions.filter(x=>ids.has(String(x.purchaseId))).map(x=>{const id=String(x.id||uid()),type=x.type==='finish'?'finish':'portion',unit=['g','kg','ml','L','개','회'].includes(x.unit)?x.unit:'회';return {...x,id,purchaseId:String(x.purchaseId),mealId:String(x.mealId||`food-meal-${id}`),type,date:/^\d{4}-\d{2}-\d{2}$/.test(x.date||'')?x.date:iso(today),servings:type==='finish'?0:1,amount:type==='finish'?0:Math.max(.01,Number(x.amount)||Number(x.servings)||1),unit,cost:Math.max(0,Number(x.cost)||0),memo:String(x.memo||'').trim(),createdAt:x.createdAt||new Date().toISOString()}});
  S.foodConsumptions.forEach(x=>{const p=S.foodPurchases.find(v=>v.id===x.purchaseId);if(!p||x.type==='finish')return;const pack=foodPackageMeasure(p);if(x.unit==='회'&&pack.dimension==='count'){x.unit=pack.unit;x.amount=Math.max(.01,Number(x.amount)||1)}});
...
function changeMonth(delta){const [y,m]=selectedMonth.split('-').map(Number),d=new Date(y,m-1+delta,1);selectedMonth=monthKey(d);periodMode='month';S.settings.periodMode='month';render()}
function setPeriodMode(mode){periodMode=mode==='month'?'month':'rolling';S.settings.periodMode=periodMode;if(periodMode==='rolling')selectedMonth=monthKey(new Date());render()}
function periodControl(){const [y,m]=selectedMonth.split('-'),rolling=periodMode==='rolling';return `<div class="period-control"><div class="period-switch" role="group" aria-label="조회 기간"><button data-period-mode="rolling" class="${rolling?'active':''}">최근 1개월</button><button data-period-mode="month" class="${rolling?'':'active'}">선택 월</button></div><div class="period-nav ${rolling?'rolling':''}"><button class="pill" data-month="-1" ${rolling?'disabled':''} aria-label="이전 달">‹</button><span class="month-label">${rolling?periodRangeLabel():`${y}년 ${Number(m)}월`}</span><button class="pill" data-month="1" ${rolling?'disabled':''} aria-label="다음 달">›</button></div></div>`}
function header(title,actions=''){const showPeriod=['dashboard','transactions','analysis','budget','food','assets'].includes(page);return `<div class="top"><div><h1>${title}</h1></div><div class="controls">${showPeriod?periodControl():''}${actions}</div></div>`}
...
function barChart(data,color='#2966ff'){const max=Math.max(...data.map(x=>x.value),1);return `<div class="bar-chart"><div class="axis-label">금액(원)</div><div class="bar-scroll" tabindex="0" aria-label="지출 추이 차트, 좌우로 스크롤할 수 있습니다"><div class="bars" style="--bar-count:${Math.max(data.length,1)}">${data.map(x=>`<div class="bar-col" title="${esc(x.label)}: ${money(x.value)}"><b>${money(x.value)}</b><div class="vbar" style="height:${Math.max(4,x.value/max*145)}px;background:${x.color||color}"></div><span>${esc(x.label)}</span></div>`).join('')}</div></div></div>`}
...
function side(){const t=totals();$('#side').innerHTML=`<h3>${periodTitle()} 요약</h3><p><b class="blue">수입</b><strong>${money(t.inc)}</strong></p><p><b class="red">지출</b><strong>${money(t.exp)}</strong></p><p><b class="green">잔액</b><strong>${money(t.sav)}</strong></p>`}
function dashboard(){const t=totals(),spend=categoryTotals(),recent=[...visibleMonthTx()].sort((a,b)=>b.date.localeCompare(a.date)).slice(0,6),tracked=S.accounts.filter(x=>x.trackBalance!==false),label=periodTitle();return header('대시보드','<button class="pill rent-toggle '+(excludeRent?'active':'')+'" id="toggleRent" aria-pressed="'+excludeRent+'" title="현재 통계 상태">'+rentFilterLabel()+'</button><button class="btn primary" id="addTx">+ 거래 추가</button>')+`<div class="grid cols4">${metric('＋','soft-blue',`${label} 수입`,money(t.inc),`${visibleMonthTx().filter(x=>x.type==='수입').length}건`)}${metric('−','soft-red',`${label} 지출`,money(t.exp),`${visibleMonthTx().filter(x=>x.type==='지출').length}건`)}${metric('₩','soft-green',`${label} 잔액`,money(t.sav),'수입 − 지출')}${metric('⌂','soft-purple','총 자산',money(tracked.reduce((a,x)=>a+balance(x),0)),`${tracked.length}개 자산`)}</div><div class="grid cols2 section-gap"><div class="card"><h3>카테고리별 지출</h3>${spend.length?`<div class="chart-pair">${donut(spend,`총 지출\n${money(t.exp)}`)}${legend(spend)}</div>`:empty(`${label} 지출이 없습니다.`)}</div><div class="card"><h3>일별 지출 추이</h3>${barChart(dailyData())}</div></div><div class="grid cols2 section-gap"><div class="card"><div class="card-head"><h3>소비 캘린더</h3><span>날짜를 누르면 거래를 확인할 수 있습니다.</span></div>${calendar()}</div><div class="card"><div class="card-head"><h3>최근 거래</h3><button class="link" data-go="transactions">전체 보기</button></div>${recent.length?recent.map(txLine).join(''):empty('거래가 없습니다.')}</div></div>`}
...
function dailyData(filter=()=>true){const dates=periodDates(),map={};visibleMonthTx().filter(x=>x.type==='지출'&&filter(x)).forEach(x=>map[x.date]=(map[x.date]||0)+Math.abs(x.amount));return dates.map(date=>({label:periodMode==='rolling'?`${Number(date.slice(5,7))}/${Number(date.slice(8))}`:`${Number(date.slice(8))}일`,value:map[date]||0})).filter(x=>x.value||dates.length<=14)}
function calendarHeatStyle(value,max){if(!value||!max)return'';const level=Math.min(1,Math.sqrt(value/max)),hue=Math.round(140*(1-level)),mix=Math.round(14+level*34);return `style="--heat-color:hsl(${hue} 72% 46%);--heat-mix:${mix}%"`}
function calendar(){const dates=periodDates(),first=new Date(`${dates[0]}T00:00:00`).getDay(),map={};calendarTx().filter(x=>x.type==='지출'&&(!excludeRent||!rentTx(x))).forEach(x=>map[x.date]=(map[x.date]||0)+Math.abs(x.amount));const max=Math.max(...Object.values(map),0);return `<div class="calendar"><div class="calendar-heat-legend"><span>적은 지출</span><i></i><span>많은 지출</span></div><div class="weekdays">${['일','월','화','수','목','금','토'].map(x=>`<b>${x}</b>`).join('')}</div><div class="cal-grid">${'<i></i>'.repeat(first)}${dates.map(date=>{const v=map[date]||0;return `<button data-date="${date}" class="${v?'spent':''}" ${calendarHeatStyle(v,max)} title="${date}"><b>${Number(date.slice(8))}</b>${periodMode==='rolling'?`<small>${Number(date.slice(5,7))}월</small>`:''}${v?`<span>${money(v)}</span>`:''}</button>`}).join('')}</div></div>`}
...
const foodMealKey=x=>String(x.mealId||`food-meal-${x.id}`);
const foodIsFinished=p=>/^\d{4}-\d{2}-\d{2}$/.test(p.finishedDate||'');
const foodHasPrice=p=>p?.priceKnown!==false;
...
const foodPurchaseDateLabel=p=>p.purchaseDate||'기존 보유 재고 · 구매일 미기록';
const foodPriceLabel=p=>foodHasPrice(p)?money(p.price):'금액 미기록';
...
function foodUnitPriceLabel(p){if(!foodHasPrice(p))return '금액 미기록';const u=foodUnitPrice(p);return `${u.value.toLocaleString('ko-KR',{minimumFractionDigits:u.value<10?2:0,maximumFractionDigits:2})}원/${u.unit}`}
function foodMealGroups(records=S.foodConsumptions){
  const groups=new Map();
  records.forEach(x=>{const key=foodMealKey(x),group=groups.get(key)||{id:key,date:x.date,createdAt:x.createdAt||'',records:[]};group.records.push(x);if(x.date<group.date)group.date=x.date;if(String(x.createdAt||'')<group.createdAt)group.createdAt=x.createdAt||'';groups.set(key,group)});
  return [...groups.values()];
}
function foodLatestEvent(p){return [...foodPortions(p)].sort((a,b)=>b.date.localeCompare(a.date)||String(b.createdAt).localeCompare(String(a.createdAt)))[0]||null}
function foodMealTitle(records){
  return records.map(x=>{const p=foodPurchase(x.purchaseId);return p?`${foodPurchaseTitle(p)} (${foodAmountLabel(x.amount||x.servings,x.unit||'회')})`:''}).filter(Boolean).join(' + ');
}
...
function recalculateFoodPurchaseCosts(p){
  ...
  if(!portions.length||!foodHasPrice(p))return;
  if(finished){
...
function foodEventCostLabel(x,p){return !foodHasPrice(p)?'금액 미기록':Number(x.cost)>0?money(x.cost):(foodIsFinished(p)?money(0):'정산 대기')}
function foodMealCostLabel(records){const priced=records.filter(x=>foodHasPrice(foodPurchase(x.purchaseId))),total=priced.reduce((sum,x)=>sum+(Number(x.cost)||0),0),hasUnknown=priced.length<records.length;return priced.length?`${money(total)}${hasUnknown?' + 금액 미기록':''}`:'금액 미기록'}
...
function foodInventory(){
  const list=S.foodPurchases.filter(p=>!foodIsFinished(p)&&foodMatches(p)).sort((a,b)=>b.purchaseDate.localeCompare(a.purchaseDate)||String(b.createdAt).localeCompare(String(a.createdAt)));
  return `<div class="card section-gap"><div class="card-head food-inventory-head"><div><h3>식재료 재고</h3><b>${list.length}개 남음</b></div><button class="btn primary" id="addFoodMeal" ${S.foodPurchases.some(x=>!foodIsFinished(x))?'':'disabled'}>+ 먹은 기록</button></div>${list.length?`<div class="food-inventory-grid">${list.map(p=>{const events=foodEvents(p).sort((a,b)=>a.date.localeCompare(b.date)||String(a.createdAt).localeCompare(String(b.createdAt))),used=foodConsumed(p),pack=foodPackageMeasure(p),measured=foodMeasuredConsumed(p),pct=pack.amount?Math.min(100,measured/pack.amount*100):0;return `<article class="food-purchase-card"><div class="food-purchase-head"><div><span>${esc(foodPurchaseDateLabel(p))} · ${esc(p.store||'구매처 미입력')}</span><h4>${esc(foodPurchaseTitle(p))}</h4></div><strong>${foodPriceLabel(p)}</strong></div><div class="food-purchase-meta"><span>총 ${foodAmountLabel(p.totalAmount||p.packageAmount,p.totalUnit||p.packageUnit)}</span><span>${foodUnitPriceLabel(p)}</span><span>${foodConsumedSummary(p)}</span></div><div class="food-progress"><i style="width:${pct}%"></i></div><div class="food-remain"><b>${foodNumber(used)}회 먹은 상태</b></div><div class="food-consume-actions"><button class="primary-mini" data-food-finish="${p.id}">다 먹었음</button></div><div class="food-card-history"><b>먹은 날짜</b>${events.length?`<div>${events.map(x=>`<button data-food-meal-edit="${foodMealKey(x)}"><span>${esc(x.date)}</span><strong>${foodAmountLabel(x.amount||x.servings,x.unit||'회')}</strong></button>`).join('')}</div>`:`<span>아직 먹은 기록이 없습니다.</span>`}</div><div class="food-card-foot"><span>${esc(p.memo)||'메모 없음'}</span><button data-food-edit="${p.id}">수정</button><button class="danger-text" data-food-delete="${p.id}">삭제</button></div></article>`}).join('')}</div>`:empty('현재 남은 재고가 없습니다.')}</div>`
}
function foodHistory(){
  const records=S.foodConsumptions.filter(x=>dateInPeriod(x.date)),list=foodMealGroups(records).map(g=>({...g,visible:g.records.filter(x=>{const p=foodPurchase(x.purchaseId);return p&&foodMatches(p)})})).filter(g=>g.visible.length).sort((a,b)=>b.date.localeCompare(a.date)||String(b.createdAt).localeCompare(String(a.createdAt)));
  return `<div class="card section-gap"><div class="card-head"><h3>먹은 기록</h3><b>${list.length}번 식사</b></div>${list.length?`<div class="food-history-list">${list.map(g=>{const auto=g.visible.length===1&&g.visible[0].autoFinished===true,reopenable=g.visible.map(x=>foodPurchase(x.purchaseId)).find(p=>p&&foodIsFinished(p)&&foodLatestEvent(p)&&foodMealKey(foodLatestEvent(p))===g.id);return `<article><time>${g.date}</time><div><b>${esc(foodMealTitle(g.visible))}</b><small>${g.visible.length}개 식재료${auto?' · 다 먹음 자동 기록':reopenable?' · 다 먹음':''}</small></div><strong>${foodMealCostLabel(g.visible)}</strong><div class="food-row-actions">${reopenable?`<button data-food-reopen="${reopenable.id}">완료 취소</button>`:''}<button data-food-meal-edit="${esc(g.id)}">수정</button><button data-food-meal-delete="${esc(g.id)}">삭제</button></div></article>`}).join('')}</div>`:empty(`${periodTitle()}에 먹은 기록이 없습니다.`)}</div>`
}
function foodItemStats(){
  const list=S.foodConsumptions.filter(x=>dateInPeriod(x.date)),map={};
  list.forEach(x=>{const p=foodPurchase(x.purchaseId);if(!p||!foodMatches(p))return;const name=foodCategory(p),key=name.toLocaleLowerCase('ko'),r=map[key]||(map[key]={name,cost:0,count:0,pricedCount:0,last:'',amounts:{}});if(foodHasPrice(p)){r.cost+=Number(x.cost)||0;r.pricedCount++}const m=foodEventMeasure(x);r.amounts[m.unit]=(r.amounts[m.unit]||0)+m.amount;r.count++;if(x.date>r.last)r.last=x.date});
  return Object.values(map).sort((a,b)=>b.cost-a.cost);
}
function foodTrend(){
  const month=S.foodConsumptions.filter(x=>{const p=foodPurchase(x.purchaseId);return dateInPeriod(x.date)&&p&&foodMatches(p)&&foodHasPrice(p)}),dates=periodDates();
  if(foodMode==='일'){const out=dates.map(date=>({label:periodMode==='rolling'?`${Number(date.slice(5,7))}/${Number(date.slice(8))}`:`${Number(date.slice(8))}일`,date,value:0})),index=new Map(out.map((x,i)=>[x.date,i]));month.forEach(x=>{const i=index.get(x.date);if(i!=null)out[i].value+=Number(x.cost)||0});return out}
  if(foodMode==='주'){const count=Math.ceil(dates.length/7),out=Array.from({length:count},(_,i)=>({label:`${i+1}주`,value:0})),start=new Date(`${dates[0]}T00:00:00`);month.forEach(x=>{const d=new Date(`${x.date}T00:00:00`),i=Math.min(count-1,Math.floor((d-start)/86400000/7));out[Math.max(0,i)].value+=Number(x.cost)||0});return out}
  const out=[];for(let n=5;n>=0;n--){const [y,m]=selectedMonth.split('-').map(Number),d=new Date(y,m-1-n,1),key=monthKey(d);out.push({label:`${d.getMonth()+1}월`,value:S.foodConsumptions.filter(x=>{const p=foodPurchase(x.purchaseId);return x.date.startsWith(key)&&p&&foodMatches(p)&&foodHasPrice(p)}).reduce((a,x)=>a+(Number(x.cost)||0),0)})}return out
}
function foodStats(){
  const list=S.foodConsumptions.filter(x=>dateInPeriod(x.date)&&foodMatches(foodPurchase(x.purchaseId)||{})),priced=list.filter(x=>foodHasPrice(foodPurchase(x.purchaseId))),unpriced=list.length-priced.length,meals=new Set(list.map(foodMealKey)).size,total=priced.reduce((a,x)=>a+(Number(x.cost)||0),0),days=periodDayCount(),stats=foodItemStats();
  return `<div class="grid cols4 section-gap food-summary-grid">${metric('🍴','soft-red',`${periodTitle()} 먹은 식비`,money(total),`${meals}번 식사${unpriced?` · 금액 미기록 ${unpriced}건 제외`:''}`)}${metric('日','soft-green','1일 평균',money(total/days),`${days}일 기준`)}${metric('週','soft-blue','주 평균',money(total/(days/7)),'가격이 있는 기록 기준')}${metric('▦','soft-orange','먹은 식품',`${stats.length}종`,'먹은 기록 전체 기준')}</div><div class="card section-gap"><div class="card-head"><h3>종류별 소비 통계</h3><span>${periodRangeLabel()}</span></div>${stats.length?`<div class="food-stats-list"><div class="food-list-head"><span>식품</span><span>횟수·양</span><span>소비금액</span><span>회당·최근</span></div>${stats.map(x=>`<article><div><b>${esc(x.name)}</b><small>${Object.entries(x.amounts).map(([unit,amount])=>foodAmountLabel(amount,unit)).join(' + ')||'-'}</small></div><span>${x.count}회</span><strong>${x.pricedCount?money(x.cost):'금액 미기록'}</strong><small>${x.pricedCount?`${money(x.cost/x.pricedCount)}/회`:''}${x.last?` · ${x.last.slice(5)}`:''}</small></article>`).join('')}</div>`:empty('먹은 기록을 추가하면 종류별 통계가 표시됩니다.')}</div><div class="card section-gap"><div class="card-head"><h3>먹은 식비 추이</h3><div class="tabs compact">${['일','주','월'].map(x=>`<button data-food-mode="${x}" class="${foodMode===x?'active':''}">${x}</button>`).join('')}</div></div>${barChart(foodTrend(),'#ff585d')}</div>`
}
function foodPrices(){
  const groups={};S.foodPurchases.filter(foodHasPrice).forEach(p=>{const category=foodCategory(p),u=foodUnitPrice(p),key=`${category.toLocaleLowerCase('ko')}|${u.dimension}`,g=groups[key]||(groups[key]={name:category,unit:u.unit,rows:[]});g.rows.push({...p,unitPrice:u.value})});
  const list=Object.values(groups).sort((a,b)=>a.name.localeCompare(b.name,'ko')),rows=list.flatMap(g=>g.rows.sort((a,b)=>a.unitPrice-b.unitPrice).map((x,i)=>({...x,compareCategory:g.name,best:i===0})));
  return `<div class="card section-gap"><div class="card-head"><div><h3>전체 상품 가격 비교</h3><p class="hint">같은 종류와 단위를 묶고 단가가 낮은 순서로 한 화면에 표시합니다.</p></div><b>${rows.length}개 상품</b></div>${rows.length?`<div class="food-compare-list"><div class="food-list-head"><span>종류·상품</span><span>구매처</span><span>총액·총량</span><span>단위 가격</span></div>${rows.map(x=>`<article class="${x.best?'best-price':''}"><div><b>${esc(x.compareCategory)}${x.best?' <em>최저</em>':''}</b><small>${esc(x.variant||'상품명 미입력')}</small></div><span>${esc(x.store)||(x.source==='existing'?'기존 재고':'미입력')}</span><small>${money(x.price)} · ${foodAmountLabel(x.totalAmount||x.packageAmount,x.totalUnit||x.packageUnit)}</small><strong>${foodUnitPriceLabel(x)}</strong></article>`).join('')}</div>`:empty('가격이 입력된 구매 기록이 있으면 전체 상품을 비교할 수 있습니다.')}</div>`
}
function food(){return header('식비 관리')+foodQuickForm()+foodTabs()+(foodTab==='prices'?'':foodCategoryTabs())+(foodTab==='history'?foodHistory():foodTab==='stats'?foodStats():foodTab==='prices'?foodPrices():foodInventory())}
function saveFoodEntry(){const name=$('#foodTitle').value.trim(),variant=$('#foodVariant').value.trim(),price=Math.abs(num($('#foodAmount').value)),totalAmount=Math.max(.01,decimalNum($('#foodTotalAmount').value)),totalUnit=$('#foodTotalUnit').value;if(!name||!price)return toast('식품 종류와 구매 가격을 입력하세요.');const purchaseDate=$('#foodDate').value||iso(today);S.foodPurchases.push({id:uid(),name,category:name,variant,source:'purchase',priceKnown:true,purchaseDate,store:$('#foodStore').value.trim(),price,totalAmount,totalUnit,packageAmount:totalAmount,packageUnit:totalUnit,quantity:1,trackingMode:'free',servings:null,finishedDate:'',memo:$('#foodMemo').value.trim(),createdAt:new Date().toISOString()});selectedMonth=purchaseDate.slice(0,7);foodCategoryFilter=name;foodTab='inventory';foodEntryExpanded=false;render();toast(`${foodPurchaseTitle(S.foodPurchases.at(-1))} 구매 기록을 저장했습니다.`)}
function existingFoodModal(){const last=foodLastPurchase(),category=last?foodCategory(last):'',variant=last?.variant||'',store=last?.store||'',amount=last?.totalAmount||last?.packageAmount||1,unit=last?.totalUnit||last?.packageUnit||'개';$('#submodal').innerHTML=`<div class="modal"><div class="modalbox"><h2>기존 재고 등록</h2><p class="hint">구매일은 기록하지 않습니다. 가격을 모르면 비워 두세요.</p><div class="modal-grid"><label>식품 종류<input id="existingFoodName" value="${esc(category)}" placeholder="예: 파스타 면"></label><label>상품·브랜드<input id="existingFoodVariant" value="${esc(variant)}" placeholder="예: 바릴라 스파게티"></label><label>기억나는 구매처 (선택)<input id="existingFoodStore" value="${esc(store)}"></label><label>기억나는 가격 (선택)<input id="existingFoodPrice" inputmode="numeric" placeholder="모르면 비워두기"></label><label>현재 등록할 총량<input id="existingFoodTotalAmount" inputmode="decimal" value="${foodNumber(amount)}"></label><label>총량 단위<select id="existingFoodTotalUnit">${foodUnitOptions(unit)}</select></label><label class="wide">메모<input id="existingFoodMemo" placeholder="선택 입력"></label></div><div class="modal-actions"><button class="pill" id="cancelExistingFood">취소</button><button class="btn primary" id="confirmExistingFood">재고 등록</button></div></div></div>`;$('#cancelExistingFood').onclick=closeSubmodal;$('#confirmExistingFood').onclick=()=>{const name=$('#existingFoodName').value.trim(),priceText=$('#existingFoodPrice').value.trim(),priceKnown=priceText!=='',price=priceKnown?Math.abs(num(priceText)):0,amountText=$('#existingFoodTotalAmount').value.trim(),totalAmount=decimalNum(amountText),totalUnit=$('#existingFoodTotalUnit').value;if(!name||!amountText||totalAmount<=0)return toast('식품 종류와 0보다 큰 총량을 입력하세요.');if(priceKnown&&!/[0-9]/.test(priceText))return toast('가격은 숫자로 입력하거나 비워 두세요.');S.foodPurchases.push({id:uid(),name,category:name,variant:$('#existingFoodVariant').value.trim(),source:'existing',priceKnown,purchaseDate:'',store:$('#existingFoodStore').value.trim(),price,totalAmount,totalUnit,packageAmount:totalAmount,packageUnit:totalUnit,quantity:1,trackingMode:'free',servings:null,finishedDate:'',memo:$('#existingFoodMemo').value.trim(),createdAt:new Date().toISOString()});foodCategoryFilter=name;foodTab='inventory';closeSubmodal();render();toast(`${foodPurchaseTitle(S.foodPurchases.at(-1))}을 기존 재고로 등록했습니다.`)}}
function saveFoodMeal(date,entries,mealId=''){
  const purchaseIds=entries.map(x=>x.purchaseId),current=mealId?S.foodConsumptions.filter(x=>foodMealKey(x)===mealId):[],currentByPurchase=new Map(current.map(x=>[x.purchaseId,x])),id=mealId||`food-meal-${uid()}`,createdAt=current[0]?.createdAt||new Date().toISOString(),affected=new Set([...current.map(x=>x.purchaseId),...purchaseIds]);
  S.foodConsumptions=S.foodConsumptions.filter(x=>foodMealKey(x)!==id);
  entries.forEach(entry=>{const purchaseId=entry.purchaseId,old=currentByPurchase.get(purchaseId),p=foodPurchase(purchaseId),pack=p?foodPackageMeasure(p):{unit:'회',dimension:'event'},amount=Math.max(.01,Number(entry.amount)||1),unit=entry.unit||pack.unit||'회';S.foodConsumptions.push(old?{...old,mealId:id,date,amount,unit}:{id:uid(),mealId:id,purchaseId,type:'portion',date,servings:1,amount,unit,cost:0,memo:'',createdAt})});
  affected.forEach(purchaseId=>recalculateFoodPurchaseCosts(foodPurchase(purchaseId)));
  selectedMonth=date.slice(0,7);closeSubmodal();render();toast(`${purchaseIds.length}개 식재료의 먹은 기록을 저장했습니다.`);
}
function foodMealModal(mealId=''){
  const current=mealId?S.foodConsumptions.filter(x=>foodMealKey(x)===mealId):[],selected=new Set(current.map(x=>x.purchaseId)),date=current[0]?.date||iso(today),choices=S.foodPurchases.filter(p=>!foodIsFinished(p)||selected.has(p.id)).sort((a,b)=>Number(foodIsFinished(a))-Number(foodIsFinished(b))||b.purchaseDate.localeCompare(a.purchaseDate));
  if(!choices.length)return toast('남아 있는 식재료가 없습니다.');
  const currentByPurchase=new Map(current.map(x=>[x.purchaseId,x]));
  $('#submodal').innerHTML=`<div class="modal"><div class="modalbox food-meal-box"><h2>${mealId?'먹은 기록 수정':'먹은 기록'}</h2><label class="food-meal-date">먹은 날짜<input id="foodMealDate" type="date" value="${date}"></label><div class="food-meal-list">${choices.map(p=>{const old=currentByPurchase.get(p.id),pack=foodPackageMeasure(p),amount=old?.amount||1,unit=old?.unit||pack.unit||'회';return `<label class="food-meal-option ${selected.has(p.id)?'selected':''}"><input type="checkbox" data-food-meal-pick value="${p.id}" ${selected.has(p.id)?'checked':''}><span><b>${esc(foodPurchaseTitle(p))}</b><small>${esc(p.store||(p.source==='existing'?'기존 보유 재고':'구매처 미입력'))} · ${foodConsumed(p)}회 먹음${foodIsFinished(p)?' · 다 먹음':''}</small></span><span class="food-meal-amount"><input data-food-meal-amount="${p.id}" inputmode="decimal" value="${foodNumber(amount)}" aria-label="${esc(foodPurchaseTitle(p))} 먹은 양"><em>${esc(unit)}</em></span></label>`}).join('')}</div><div class="modal-actions"><button class="pill" id="cancelFoodMeal">취소</button><button class="btn primary" id="confirmFoodMeal">${mealId?'저장':'기록'}</button></div></div></div>`;
  const update=()=>$$('[data-food-meal-pick]').forEach(x=>x.closest('.food-meal-option').classList.toggle('selected',x.checked));$$('[data-food-meal-pick]').forEach(x=>x.onchange=update);update();
  $('#cancelFoodMeal').onclick=closeSubmodal;$('#confirmFoodMeal').onclick=()=>{const chosen=$$('[data-food-meal-pick]:checked').map(x=>{const p=foodPurchase(x.value),pack=foodPackageMeasure(p),amount=decimalNum($(`[data-food-meal-amount="${CSS.escape(x.value)}"]`).value);return {purchaseId:x.value,amount,unit:pack.unit||'회'}}),chosenDate=$('#foodMealDate').value;if(!/^\d{4}-\d{2}-\d{2}$/.test(chosenDate))return toast('먹은 날짜를 선택하세요.');if(!chosen.length)return toast('먹은 식재료를 하나 이상 선택하세요.');if(chosen.some(x=>x.amount<=0))return toast('먹은 양은 0보다 크게 입력하세요.');saveFoodMeal(chosenDate,chosen,mealId)};
}
function finishFood(p,date=iso(new Date())){if(!p||foodIsFinished(p))return false;p.finishedDate=date;if(!foodPortions(p).length){const pack=foodPackageMeasure(p),mealId=`food-meal-finished-${uid()}`;S.foodConsumptions.push({id:uid(),mealId,purchaseId:p.id,type:'portion',date,servings:1,amount:Math.max(.01,pack.amount||1),unit:pack.unit||'회',cost:0,memo:'다 먹음 자동 기록',autoFinished:true,createdAt:new Date().toISOString()})}recalculateFoodPurchaseCosts(p);return true}
function finishFoodModal(id){const p=foodPurchase(id);if(!p||foodIsFinished(p))return;confirmDialog({title:'다 먹음',message:`${foodPurchaseTitle(p)}을 다 먹은 것으로 처리할까요?`,details:['먹은 기록이 하나도 없으면 오늘 남은 전량을 먹은 기록으로 자동 남깁니다.'],confirmText:'확인',onConfirm:()=>{finishFood(p);render();toast(`${foodPurchaseTitle(p)}을 다 먹은 기록으로 남겼습니다.`)}})}
function reopenFood(id){const p=foodPurchase(id);if(!p)return;S.foodConsumptions=S.foodConsumptions.filter(x=>!(x.purchaseId===id&&x.autoFinished===true));p.finishedDate='';recalculateFoodPurchaseCosts(p);foodTab='inventory';render();toast('다 먹음 상태와 자동 기록을 취소했습니다.')}
function foodConsumptionEditModal(id){const x=S.foodConsumptions.find(v=>v.id===id);if(x)foodMealModal(foodMealKey(x))}
function deleteFoodMeal(mealId){const records=S.foodConsumptions.filter(x=>foodMealKey(x)===mealId),affected=new Set(records.map(x=>x.purchaseId));S.foodConsumptions=S.foodConsumptions.filter(x=>foodMealKey(x)!==mealId);affected.forEach(id=>recalculateFoodPurchaseCosts(foodPurchase(id)));render();toast('먹은 기록을 삭제했습니다.')}
...
function foodEditModal(id){const p=foodPurchase(id);if(!p)return;const existing=p.source==='existing';$('#submodal').innerHTML=`<div class="modal"><div class="modalbox"><h2>${existing?'기존 재고 수정':'식품 구매 기록 수정'}</h2>${existing?'<p class="hint">구매일은 기록되지 않습니다. 가격을 비우면 금액 계산에서 제외됩니다.</p>':''}<div class="modal-grid">${existing?'':`<label>구매일<input id="editFoodDate" type="date" value="${p.purchaseDate}"></label>`}<label>식품 종류<input id="editFoodName" value="${esc(foodCategory(p))}"></label><label>상품·브랜드<input id="editFoodVariant" value="${esc(p.variant)}"></label><label>마트·구매처<input id="editFoodStore" value="${esc(p.store)}"></label><label>${existing?'기억나는 가격 (선택)':'구매 가격'}<input id="editFoodPrice" inputmode="numeric" value="${foodHasPrice(p)?p.price:''}" ${existing?'placeholder="모르면 비워두기"':''}></label><label>구매 총량<input id="editFoodTotalAmount" inputmode="decimal" value="${p.totalAmount||p.packageAmount}"></label><label>총량 단위<select id="editFoodTotalUnit">${foodUnitOptions(p.totalUnit||p.packageUnit)}</select></label><label class="wide">메모<input id="editFoodMemo" value="${esc(p.memo)}"></label></div><div class="modal-actions"><button class="pill" id="cancelFoodEdit">취소</button><button class="btn primary" id="confirmFoodEdit">저장</button></div></div></div>`;$('#cancelFoodEdit').onclick=closeSubmodal;$('#confirmFoodEdit').onclick=()=>{const name=$('#editFoodName').value.trim(),priceText=$('#editFoodPrice').value.trim(),priceKnown=existing?priceText!=='':true,price=Math.abs(num(priceText)),amountText=$('#editFoodTotalAmount').value.trim(),totalAmount=decimalNum(amountText),totalUnit=$('#editFoodTotalUnit').value;if(!name||!amountText||totalAmount<=0)return toast('식품 종류와 0보다 큰 총량을 입력하세요.');if(existing&&priceKnown&&!/[0-9]/.test(priceText))return toast('가격은 숫자로 입력하거나 비워 두세요.');if(!existing&&!price)return toast('식품 종류와 가격을 입력하세요.');Object.assign(p,{purchaseDate:existing?'':$('#editFoodDate').value,name,category:name,variant:$('#editFoodVariant').value.trim(),store:$('#editFoodStore').value.trim(),priceKnown,price:priceKnown?price:0,totalAmount,totalUnit,packageAmount:totalAmount,packageUnit:totalUnit,quantity:1,memo:$('#editFoodMemo').value.trim()});recalculateFoodPurchaseCosts(p);foodCategoryFilter=name;closeSubmodal();render();toast(existing?'기존 재고를 수정했습니다.':'구매 기록을 수정했습니다.')}}
...
function settings(){const fontSize=S.settings.fontSize||'normal';return header('설정')+`<div class="grid cols2"><div class="card"><div class="card-head"><h3>카테고리 관리</h3><button class="btn primary" id="manageCats">관리창 열기</button></div><p class="hint">영역·분류·항목을 별도 관리창에서 추가·변경·삭제할 수 있습니다.</p><div class="category-summary">${areas().map(a=>`<p><b>${a}</b><span>${sections(a).length}개 분류 · ${S.categories.filter(x=>x.area===a).length}개 항목</span></p>`).join('')}</div></div><div class="card"><div class="card-head"><h3>자산 설정</h3><button class="btn primary" data-go="assets">자산 관리로 이동</button></div><p class="hint">계좌·카드·현금·전자지갑은 모두 자산 관리에서 관리합니다.</p><div class="summary-lines"><p><span>전체 자산 항목</span><b>${S.accounts.length}개</b></p><p><span>거래 사용 가능 자산</span><b>${paymentOptions().length}개</b></p></div></div><div class="card"><h3>화면 및 저장</h3><div class="manage-row"><span>다크 모드</span><button class="pill" id="toggleDark">${S.settings.dark?'켜짐':'꺼짐'}</button></div><div class="manage-row font-size-row"><span><b>글씨 크기</b><small>글씨와 칸이 함께 커집니다.</small></span><div class="font-size-control" role="group" aria-label="글씨 크기"><button class="pill ${fontSize==='normal'?'active':''}" data-font-size="normal">기본</button><button class="pill ${fontSize==='large'?'active':''}" data-font-size="large">크게</button><button class="pill ${fontSize==='xlarge'?'active':''}" data-font-size="xlarge">아주 크게</button></div></div><div class="manage-row"><span>로컬 자동 저장</span><b>변경 후 저장</b></div><p class="hint">처음 실행해 화면만 열린 상태에서는 빈 데이터를 덮어쓰지 않습니다. 거래·설정 등 변경이 생긴 뒤부터 로컬에 저장됩니다.</p></div><div class="card"><h3>데이터 관리</h3><div class="data-actions"><button class="pill" id="exportData">백업 파일 저장</button><button class="pill" id="importDataButton">백업 복원</button><input type="file" id="importData" accept="application/json" hidden><button class="pill danger" id="resetData">전체 초기화</button></div></div></div>`}
...
function render(){syncSubscriptions();ensureStocks();applyGoogleAutoPreference();applyPeriodPreference();applyFontPreference();const chosen=S.settings.theme||(S.settings.dark?'dark':'light'),resolved=chosen==='system'?(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light'):chosen;document.documentElement.dataset.theme=resolved;document.documentElement.classList.toggle('dark',['dark','midnight'].includes(resolved));side();$('#app').dataset.page=page;$('#app').innerHTML=pages[page]();const secondary=['budget','subscriptions','food','assets','stocks','settings'];$$('#nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===page||(b.dataset.page==='more'&&secondary.includes(page))));bind();if(appReady)save()}
function bind(){
  $('#toggleRent')?.addEventListener('click',()=>{excludeRent=!excludeRent;render()});
  $('#googleSetupHelp')?.addEventListener('click',googleSetupDialog);
  $('#refreshGoogleBackups')?.addEventListener('click',()=>{googleBackupPage=0;loadGoogleBackups()});
  $('#prevGoogleBackups')?.addEventListener('click',()=>{if(googleBackupPage>0){googleBackupPage--;render()}});
  $('#nextGoogleBackups')?.addEventListener('click',()=>{if((googleBackupPage+1)*GOOGLE_BACKUP_PAGE_SIZE<googleBackups.length){googleBackupPage++;render()}});
  ...
  $$('[data-period-mode]').forEach(b=>b.onclick=()=>setPeriodMode(b.dataset.periodMode));
  ...
  $$('[data-date]').forEach(b=>b.onclick=()=>dayModalDate(b.dataset.date));
  ...
  $('#toggleFoodEntry')?.addEventListener('click',()=>{foodEntryExpanded=!foodEntryExpanded;render()});
  $('#addExistingFood')?.addEventListener('click',existingFoodModal);
  $('#saveFoodEntry')?.addEventListener('click',saveFoodEntry);
  $('#addFoodMeal')?.addEventListener('click',()=>foodMealModal());
  ...
  $$('[data-food-meal-edit]').forEach(b=>b.onclick=()=>foodMealModal(b.dataset.foodMealEdit));
  $$('[data-food-meal-delete]').forEach(b=>b.onclick=()=>deleteFoodMeal(b.dataset.foodMealDelete));
  ...
  $$('[data-font-size]').forEach(button=>button.onclick=()=>{S.settings.fontSize=button.dataset.fontSize;render();toast('글씨 크기를 적용했습니다.')});
...
function closeTopModal(){if($('#submodal')?.children.length){closeSubmodal();return true}if($('#modal')?.children.length){closeModal();return true}return false}
function ensureModalCloseButtons(){[['#modal',closeModal],['#submodal',closeSubmodal]].forEach(([selector,closer])=>{const box=$(selector+' .modalbox');if(!box||box.querySelector('.modal-title .close,.modal-floating-close'))return;const button=document.createElement('button');button.type='button';button.className='close modal-floating-close';button.setAttribute('aria-label','창 닫기');button.textContent='×';button.onclick=closer;box.prepend(button)})}
function bindBackdropClose(rootSelector,closer){const overlay=$(rootSelector+' .modal');if(!overlay)return;const closeOnBackdrop=e=>{if(e.target===overlay)closer()};overlay.addEventListener('mousedown',closeOnBackdrop);overlay.addEventListener('click',closeOnBackdrop)}
...
function dayModalDate(key){if(!/^\d{4}-\d{2}-\d{2}$/.test(key))return;const list=S.transactions.filter(x=>x.date===key),day=Number(key.slice(8)),month=Number(key.slice(5,7));$('#modal').innerHTML=modal(`${month}월 ${day}일 거래`,list.length?list.map(txLine).join(''):empty('이 날짜의 거래가 없습니다.'),'거래 추가');modalBase();$('#saveModal').onclick=()=>{closeModal();rememberTransactionDate(key);txModal(null,'지출')}}
...
const modalObserver=new MutationObserver(ensureModalCloseButtons);
['modal','submodal'].forEach(id=>{const host=document.getElementById(id);if(host)modalObserver.observe(host,{childList:true,subtree:true})});
document.addEventListener('click',event=>{const overlay=event.target instanceof Element?event.target.closest('.modal'):null;if(!overlay||event.target!==overlay)return;overlay.parentElement?.id==='submodal'?closeSubmodal():closeModal()});
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&closeTopModal())event.preventDefault()});
document.addEventListener('backbutton',event=>{if(closeTopModal())event.preventDefault?.()});

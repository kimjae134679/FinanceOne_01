...
let page='dashboard', selectedMonth=monthKey(today), txFilter='전체', lastTransactionDate=iso(today), lastTransactionDateAt=0, foodMode='일', foodTab='inventory', foodCategoryFilter='전체', foodEntryExpanded=false, analysisMode='overview', analysisArea='지출', analysisSection='식비', analysisItem='외식/배달', excludeRent=false;
...
function calendar(){const [y,m]=selectedMonth.split('-').map(Number),first=new Date(y,m-1,1).getDay(),days=new Date(y,m,0).getDate(),map={};monthTx().filter(x=>x.type==='지출').forEach(x=>map[Number(x.date.slice(8))]=(map[Number(x.date.slice(8))]||0)+Math.abs(x.amount));const max=Math.max(...Object.values(map),0);return `<div class="calendar"><div class="calendar-heat-legend"><span>적은 지출</span><i></i><span>많은 지출</span></div><div class="weekdays">${['일','월','화','수','목','금','토'].map(x=>`<b>${x}</b>`).join('')}</div><div class="cal-grid">${'<i></i>'.repeat(first)}${Array.from({length:days},(_,i)=>{const d=i+1,v=map[d]||0;return `<button data-day="${d}" class="${v?'spent':''}" ${calendarHeatStyle(v,max)}><b>${d}</b>${v?`<span>${money(v)}</span>`:''}</button>`}).join('')}</div></div>`}
...
function foodHistory(){
  const records=S.foodConsumptions.filter(x=>x.date.startsWith(selectedMonth)),list=foodMealGroups(records).map(g=>({...g,visible:g.records.filter(x=>{const p=foodPurchase(x.purchaseId);return p&&foodMatches(p)})})).filter(g=>g.visible.length).sort((a,b)=>b.date.localeCompare(a.date)||String(b.createdAt).localeCompare(String(a.createdAt)));
  return `<div class="card section-gap"><div class="card-head"><h3>먹은 기록</h3><b>${list.length}번 식사</b></div>${list.length?`<div class="table-wrap"><table class="table responsive-table food-ledger"><thead><tr><th>날짜</th><th>함께 먹은 식재료</th><th>식재료 수</th><th>계산 식비</th><th></th></tr></thead><tbody>${list.map(g=>`<tr><td data-label="날짜">${g.date}</td><td data-label="함께 먹은 식재료"><b>${esc(foodMealTitle(g.visible))}</b></td><td data-label="식재료 수">${g.visible.length}개</td><td data-label="계산 식비" class="red"><b>${foodMealCostLabel(g.visible)}</b></td><td data-label="관리"><button data-food-meal-edit="${esc(g.id)}">수정</button><button data-food-meal-delete="${esc(g.id)}">삭제</button></td></tr>`).join('')}</tbody></table></div>`:empty('선택한 달에 먹은 기록이 없습니다.')}</div>`
}
...
function foodTrend(){
  const month=S.foodConsumptions.filter(x=>{const p=foodPurchase(x.purchaseId);return x.date.startsWith(selectedMonth)&&p&&foodMatches(p)&&foodHasPrice(p)});
...
function foodStats(){
  const list=S.foodConsumptions.filter(x=>x.date.startsWith(selectedMonth)&&foodMatches(foodPurchase(x.purchaseId)||{})),priced=list.filter(x=>foodHasPrice(foodPurchase(x.purchaseId))),unpriced=list.length-priced.length,meals=new Set(list.map(foodMealKey)).size,total=priced.reduce((a,x)=>a+(Number(x.cost)||0),0),[y,m]=selectedMonth.split('-').map(Number),days=new Date(y,m,0).getDate(),stats=foodItemStats();
  return `<div class="grid cols4 section-gap">${metric('🍴','soft-red','이번 달 먹은 식비',money(total),`${meals}번 식사${unpriced?` · 금액 미기록 ${unpriced}건 제외`:''}`)}${metric('日','soft-green','1일 평균',money(total/days),`${days}일 기준`)}${metric('週','soft-blue','주 평균',money(total/(days/7)),'가격이 있는 기록 기준')}${metric('▦','soft-orange','먹은 식품',`${stats.length}종`,'먹은 기록 전체 기준')}</div><div class="card section-gap"><div class="card-head"><h3>종류별 소비 통계</h3></div>${stats.length?`<div class="table-wrap"><table class="table food-stats-table"><thead><tr><th>식품 종류</th><th>먹은 횟수</th><th>먹은 양</th><th>월 식비</th><th>회당 평균</th><th>최근 섭취</th></tr></thead><tbody>${stats.map(x=>`<tr><td><b>${esc(x.name)}</b></td><td>${x.count}회</td><td>${Object.entries(x.amounts).map(([unit,amount])=>foodAmountLabel(amount,unit)).join(' + ')||'-'}</td><td class="red"><b>${x.pricedCount?money(x.cost):'금액 미기록'}</b></td><td>${x.pricedCount?money(x.cost/x.pricedCount):'-'}</td><td>${x.last}</td></tr>`).join('')}</tbody></table></div>`:empty('먹은 기록을 추가하면 종류별 통계가 표시됩니다.')}</div><div class="card section-gap"><div class="card-head"><h3>먹은 식비 추이</h3><div class="tabs compact">${['일','주','월'].map(x=>`<button data-food-mode="${x}" class="${foodMode===x?'active':''}">${x}</button>`).join('')}</div></div>${barChart(foodTrend(),'#ff585d')}</div>`
}
function foodPrices(){
  const groups={};S.foodPurchases.filter(p=>foodMatches(p)&&foodHasPrice(p)).forEach(p=>{const category=foodCategory(p),u=foodUnitPrice(p),key=`${category.toLocaleLowerCase('ko')}|${u.dimension}`,g=groups[key]||(groups[key]={name:category,unit:u.unit,rows:[]});g.rows.push({...p,unitPrice:u.value})});
  const list=Object.values(groups).sort((a,b)=>a.name.localeCompare(b.name,'ko'));
  return `<div class="card section-gap"><div class="card-head"><h3>종류·상품·마트별 가격 비교</h3></div>${list.length?`<div class="food-price-groups">${list.map(g=>{const rows=g.rows.sort((a,b)=>a.unitPrice-b.unitPrice),best=rows[0];return `<section class="food-price-group"><div><h4>${esc(g.name)}</h4><span>최저 ${foodUnitPriceLabel(best)} · ${esc(best.variant||best.store||'상품명 미입력')}</span></div><div class="table-wrap"><table class="table food-price-table"><thead><tr><th>상품·브랜드</th><th>마트·구매처</th><th>구매일</th><th>총 가격</th><th>구매 총량</th><th>단위 가격</th></tr></thead><tbody>${rows.map((x,i)=>`<tr class="${i===0?'best-price':''}"><td><b>${esc(x.variant||x.name)}</b>${i===0?' <span class="tag soft-green">최저가</span>':''}</td><td>${esc(x.store)||'-'}</td><td>${foodPurchaseDateLabel(x)}</td><td>${money(x.price)}</td><td>${foodAmountLabel(x.totalAmount||x.packageAmount,x.totalUnit||x.packageUnit)}</td><td><b>${foodUnitPriceLabel(x)}</b></td></tr>`).join('')}</tbody></table></div></section>`}).join('')}</div>`:empty('가격이 입력된 구매 기록이 있으면 종류·상품·마트별 단가를 비교할 수 있습니다.')}</div>`
}
...
function saveFoodMeal(date,purchaseIds,mealId=''){
  ...
  purchaseIds.forEach(purchaseId=>{const old=currentByPurchase.get(purchaseId);S.foodConsumptions.push(old?{...old,mealId:id,date}:{id:uid(),mealId:id,purchaseId,type:'portion',date,servings:1,amount:1,unit:'회',cost:0,memo:'',createdAt})});

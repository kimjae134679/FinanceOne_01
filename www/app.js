...
let page='dashboard', selectedMonth=monthKey(today), txFilter='전체', lastTransactionDate=iso(today), foodMode='일', foodTab='inventory', analysisMode='overview', analysisArea='지출', analysisSection='식비', analysisItem='외식/배달', excludeRent=false;
...
function normalizeFoodData(){
  ...
  S.foodPurchases=S.foodPurchases.map(x=>({...x,id:String(x.id||uid()),name:String(x.name||'이름 없음').trim()||'이름 없음',purchaseDate:/^\d{4}-\d{2}-\d{2}$/.test(x.purchaseDate||'')?x.purchaseDate:iso(today),store:String(x.store||'').trim(),price:Math.max(0,Number(x.price)||0),quantity:Math.max(.01,Number(x.quantity)||1),servings:Math.max(.01,Number(x.servings)||1),memo:String(x.memo||'').trim(),createdAt:x.createdAt||new Date().toISOString()}));
  const ids=new Set(S.foodPurchases.map(x=>x.id));
  S.foodConsumptions=S.foodConsumptions.filter(x=>ids.has(String(x.purchaseId))).map(x=>({...x,id:String(x.id||uid()),purchaseId:String(x.purchaseId),date:/^\d{4}-\d{2}-\d{2}$/.test(x.date||'')?x.date:iso(today),servings:Math.max(0,Number(x.servings)||0),cost:Math.max(0,Number(x.cost)||0),memo:String(x.memo||'').trim(),createdAt:x.createdAt||new Date().toISOString()})).filter(x=>x.servings>0);
}
...
const foodConsumed=p=>S.foodConsumptions.filter(x=>x.purchaseId===p.id).reduce((a,x)=>a+Number(x.servings||0),0);
const foodRemaining=p=>Math.max(0,Number(p.servings)-foodConsumed(p));
const foodCostFor=(p,servings)=>Number(p.price)*(Number(servings)||0)/Math.max(Number(p.servings)||1,.01);
const foodPurchase=id=>S.foodPurchases.find(x=>x.id===id);
function foodQuickForm(){return `<div class="card food-entry"><div class="card-head"><div><h3>식품 구매 기록</h3><p class="hint">이 기록은 거래·예산·자산과 완전히 분리된 식비 전용 장부에만 저장됩니다.</p></div></div><div class="food-form food-purchase-form"><label>구매일<input id="foodDate" type="date" value="${iso(today)}"></label><label>식품명<input id="foodTitle" placeholder="예: 닭갈비, 요구르트, 계란"></label><label>구매 가격<input id="foodAmount" inputmode="numeric" placeholder="예: 12000"></label><label>마트·구매처<input id="foodStore" placeholder="예: 이마트, 쿠팡"></label><label>구매 수량<input id="foodQuantity" inputmode="decimal" value="1" placeholder="묶음·개수"></label><label>총 먹을 분량<input id="foodServings" inputmode="decimal" value="1" placeholder="예: 3"></label><label class="food-memo">메모<input id="foodMemo" placeholder="용량, 브랜드 등 선택 입력"></label></div><div class="food-save-row"><span>구매 수량은 가격 비교에, 먹을 분량은 1/3·1회 섭취 계산에 사용됩니다.</span><button class="btn primary" id="saveFoodEntry">+ 구매 기록 저장</button></div></div>`}
...
function foodInventory(){
  const list=[...S.foodPurchases].sort((a,b)=>foodRemaining(b)-foodRemaining(a)||b.purchaseDate.localeCompare(a.purchaseDate));
  return `<div class="card section-gap"><div class="card-head"><div><h3>보유 식품과 먹기 기록</h3><p class="hint">먹은 만큼만 비용으로 계산됩니다. 남은 양을 모두 기록하면 자동으로 ‘다 먹음’이 됩니다.</p></div><b>${list.filter(x=>foodRemaining(x)>0).length}개 보유</b></div>${list.length?`<div class="food-inventory-grid">${list.map(p=>{const used=foodConsumed(p),remaining=foodRemaining(p),pct=Math.min(100,used/p.servings*100);return `<article class="food-purchase-card ${remaining<=.0001?'finished':''}"><div class="food-purchase-head"><div><span>${esc(p.purchaseDate)} · ${esc(p.store||'구매처 미입력')}</span><h4>${esc(p.name)}</h4></div><strong>${money(p.price)}</strong></div><div class="food-purchase-meta"><span>구매 ${p.quantity}개</span><span>총 ${p.servings}회분</span><span>회당 ${money(p.price/p.servings)}</span></div><div class="food-progress"><i style="width:${pct}%"></i></div><div class="food-remain"><b>${remaining<=.0001?'다 먹음':`${remaining.toFixed(2).replace(/\.00$/,'')}회분 남음`}</b><span>${used.toFixed(2).replace(/\.00$/,'')} / ${p.servings}회분 섭취</span></div>${remaining>.0001?`<div class="food-consume-actions"><button data-food-consume="${p.id}" data-amount="${p.servings/4}">1/4 먹음</button><button data-food-consume="${p.id}" data-amount="${p.servings/3}">1/3 먹음</button><button data-food-consume="${p.id}" data-amount="${p.servings/2}">1/2 먹음</button><button data-food-consume="${p.id}" data-amount="1">1회 먹음</button><button class="primary-mini" data-food-finish="${p.id}">다 먹음</button><button data-food-custom="${p.id}">직접 입력</button></div>`:''}<div class="food-card-foot"><span>${esc(p.memo)||'메모 없음'}</span><button data-food-edit="${p.id}">수정</button><button class="danger-text" data-food-delete="${p.id}">삭제</button></div></article>`}).join('')}</div>`:empty('위에서 산 음식과 가격을 먼저 기록해 보세요.')}</div>`
}
function foodHistory(){
  ...
  return `<div class="card section-gap"><div class="card-head"><div><h3>먹은 기록</h3><p class="hint">삭제하면 해당 분량이 다시 재고로 돌아옵니다.</p></div><b>${list.length}건</b></div>${list.length?`<div class="table-wrap"><table class="table responsive-table food-ledger"><thead><tr><th>날짜</th><th>식품</th><th>먹은 양</th><th>계산 식비</th><th>구매처</th><th>메모</th><th></th></tr></thead><tbody>${list.map(x=>`<tr><td data-label="날짜">${x.date}</td><td data-label="식품"><b>${esc(x.purchase.name)}</b></td><td data-label="먹은 양">${x.servings.toFixed(2).replace(/\.00$/,'')}회분</td><td data-label="계산 식비" class="red"><b>${money(x.cost)}</b></td><td data-label="구매처">${esc(x.purchase.store)||'-'}</td><td data-label="메모">${esc(x.memo)||'-'}</td><td data-label="관리"><button data-food-consumption-delete="${x.id}">삭제</button></td></tr>`).join('')}</tbody></table></div>`:empty('선택한 달에 먹은 기록이 없습니다.')}</div>`
}
function foodItemStats(){
  const list=S.foodConsumptions.filter(x=>x.date.startsWith(selectedMonth)),map={};
  list.forEach(x=>{const p=foodPurchase(x.purchaseId);if(!p)return;const key=p.name.trim().toLocaleLowerCase('ko'),r=map[key]||(map[key]={name:p.name,cost:0,servings:0,count:0,last:''});r.cost+=Number(x.cost)||foodCostFor(p,x.servings);r.servings+=Number(x.servings);r.count++;if(x.date>r.last)r.last=x.date});
...
function foodPrices(){
  ...
  return `<div class="card section-gap"><div class="card-head"><div><h3>마트별 가격 비교</h3><p class="hint">같은 식품명의 구매 기록을 수량당 가격으로 비교합니다.</p></div></div>${list.length?`<div class="food-price-groups">${list.map(g=>{const rows=g.rows.sort((a,b)=>a.unitPrice-b.unitPrice),best=rows[0];return `<section class="food-price-group"><div><h4>${esc(g.name)}</h4><span>최저 ${money(best.unitPrice)} / 1개 · ${esc(best.store||'구매처 미입력')}</span></div><div class="table-wrap"><table class="table"><thead><tr><th>구매처</th><th>구매일</th><th>총 가격</th><th>수량</th><th>개당 가격</th></tr></thead><tbody>${rows.map((x,i)=>`<tr class="${i===0?'best-price':''}"><td><b>${esc(x.store||'구매처 미입력')}</b>${i===0?' <span class="tag soft-green">최저가</span>':''}</td><td>${x.purchaseDate}</td><td>${money(x.price)}</td><td>${x.quantity}</td><td><b>${money(x.unitPrice)}</b></td></tr>`).join('')}</tbody></table></div></section>`}).join('')}</div>`:empty('구매처와 수량을 기록하면 식품별 가격을 비교할 수 있습니다.')}</div>`
}
...
function saveFoodEntry(){const name=$('#foodTitle').value.trim(),price=Math.abs(num($('#foodAmount').value)),quantity=Math.max(.01,decimalNum($('#foodQuantity').value)),servings=Math.max(.01,decimalNum($('#foodServings').value));if(!name||!price)return toast('식품명과 구매 가격을 입력하세요.');const purchaseDate=$('#foodDate').value||iso(today);S.foodPurchases.push({id:uid(),name,purchaseDate,store:$('#foodStore').value.trim(),price,quantity,servings,memo:$('#foodMemo').value.trim(),createdAt:new Date().toISOString()});selectedMonth=purchaseDate.slice(0,7);foodTab='inventory';render();toast(`${name} 구매 기록을 저장했습니다.`)}
function consumeFood(id,amount,date=iso(today),memo=''){const p=foodPurchase(id);if(!p)return;const servings=Math.min(foodRemaining(p),Math.max(0,Number(amount)||0));if(servings<=.0001)return toast('남은 분량이 없습니다.');S.foodConsumptions.push({id:uid(),purchaseId:p.id,date,servings,cost:foodCostFor(p,servings),memo,createdAt:new Date().toISOString()});selectedMonth=date.slice(0,7);render();toast(`${p.name} ${servings.toFixed(2).replace(/\.00$/,'')}회분을 먹은 것으로 기록했습니다.`)}
function foodConsumeModal(id){const p=foodPurchase(id);if(!p)return;$('#submodal').innerHTML=`<div class="modal"><div class="modalbox"><h2>${esc(p.name)} 먹은 양 기록</h2><div class="modal-grid"><label>날짜<input id="foodConsumeDate" type="date" value="${iso(today)}"></label><label>먹은 분량<input id="foodConsumeAmount" inputmode="decimal" value="${Math.min(1,foodRemaining(p))}"><small>남은 ${foodRemaining(p).toFixed(2).replace(/\.00$/,'')}회분</small></label><label class="wide">메모<input id="foodConsumeMemo" placeholder="예: 저녁 식사"></label></div><div class="modal-actions"><button class="pill" id="cancelFoodConsume">취소</button><button class="btn primary" id="confirmFoodConsume">기록</button></div></div></div>`;$('#cancelFoodConsume').onclick=closeSubmodal;$('#confirmFoodConsume').onclick=()=>{const amount=decimalNum($('#foodConsumeAmount').value);if(amount<=0||amount>foodRemaining(p)+.0001)return toast(`0보다 크고 남은 ${foodRemaining(p).toFixed(2)}회분 이하여야 합니다.`);const date=$('#foodConsumeDate').value,memo=$('#foodConsumeMemo').value.trim();closeSubmodal();consumeFood(id,amount,date,memo)}}
function foodEditModal(id){const p=foodPurchase(id);if(!p)return;$('#submodal').innerHTML=`<div class="modal"><div class="modalbox"><h2>식품 구매 기록 수정</h2><div class="modal-grid"><label>구매일<input id="editFoodDate" type="date" value="${p.purchaseDate}"></label><label>식품명<input id="editFoodName" value="${esc(p.name)}"></label><label>구매 가격<input id="editFoodPrice" inputmode="numeric" value="${p.price}"></label><label>마트·구매처<input id="editFoodStore" value="${esc(p.store)}"></label><label>구매 수량<input id="editFoodQuantity" inputmode="decimal" value="${p.quantity}"></label><label>총 먹을 분량<input id="editFoodServings" inputmode="decimal" value="${p.servings}"></label><label class="wide">메모<input id="editFoodMemo" value="${esc(p.memo)}"></label></div><div class="modal-actions"><button class="pill" id="cancelFoodEdit">취소</button><button class="btn primary" id="confirmFoodEdit">저장</button></div></div></div>`;$('#cancelFoodEdit').onclick=closeSubmodal;$('#confirmFoodEdit').onclick=()=>{const servings=Math.max(.01,decimalNum($('#editFoodServings').value));if(servings+0.0001<foodConsumed(p))return toast(`이미 ${foodConsumed(p).toFixed(2)}회분을 먹어 총 분량을 더 작게 바꿀 수 없습니다.`);const name=$('#editFoodName').value.trim(),price=Math.abs(num($('#editFoodPrice').value));if(!name||!price)return toast('식품명과 가격을 입력하세요.');Object.assign(p,{purchaseDate:$('#editFoodDate').value,name,price,store:$('#editFoodStore').value.trim(),quantity:Math.max(.01,decimalNum($('#editFoodQuantity').value)),servings,memo:$('#editFoodMemo').value.trim()});closeSubmodal();render();toast('구매 기록을 수정했습니다.')}}
...
function bind(){
  ...
  $$('[data-food-consume]').forEach(b=>b.onclick=()=>consumeFood(b.dataset.foodConsume,Number(b.dataset.amount)));
  $$('[data-food-finish]').forEach(b=>b.onclick=()=>consumeFood(b.dataset.foodFinish,foodRemaining(foodPurchase(b.dataset.foodFinish))));
  $$('[data-food-custom]').forEach(b=>b.onclick=()=>foodConsumeModal(b.dataset.foodCustom));
  ...
  $$('[data-food-consumption-delete]').forEach(b=>b.onclick=()=>{S.foodConsumptions=S.foodConsumptions.filter(x=>x.id!==b.dataset.foodConsumptionDelete);render();toast('먹은 기록을 삭제해 분량을 재고로 돌렸습니다.')});

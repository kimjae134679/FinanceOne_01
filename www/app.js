...
let page='dashboard', selectedMonth=monthKey(today), txFilter='전체', lastTransactionDate=iso(today), lastTransactionDateAt=0, foodMode='일', foodTab='inventory', analysisMode='overview', analysisArea='지출', analysisSection='식비', analysisItem='외식/배달', excludeRent=false;
...
function normalizeFoodData(){
  ...
  S.foodPurchases=S.foodPurchases.map(x=>{const trackingMode=x.trackingMode==='until-finished'?'until-finished':'fixed';return {...x,id:String(x.id||uid()),name:String(x.name||'이름 없음').trim()||'이름 없음',purchaseDate:/^\d{4}-\d{2}-\d{2}$/.test(x.purchaseDate||'')?x.purchaseDate:iso(today),store:String(x.store||'').trim(),price:Math.max(0,Number(x.price)||0),quantity:Math.max(.01,Number(x.quantity)||1),trackingMode,servings:trackingMode==='fixed'?Math.max(.01,Number(x.servings)||1):null,memo:String(x.memo||'').trim(),createdAt:x.createdAt||new Date().toISOString()}});
  const ids=new Set(S.foodPurchases.map(x=>x.id));
  S.foodConsumptions=S.foodConsumptions.filter(x=>ids.has(String(x.purchaseId))).map(x=>{const type=x.type==='finish'?'finish':'portion';return {...x,id:String(x.id||uid()),purchaseId:String(x.purchaseId),type,date:/^\d{4}-\d{2}-\d{2}$/.test(x.date||'')?x.date:iso(today),servings:type==='finish'?Math.max(0,Number(x.servings)||0):Math.max(.01,Number(x.servings)||1),cost:Math.max(0,Number(x.cost)||0),memo:String(x.memo||'').trim(),createdAt:x.createdAt||new Date().toISOString()}}).filter(x=>x.type==='finish'||x.servings>0);
  S.foodPurchases.filter(x=>x.trackingMode==='until-finished').forEach(recalculateFoodPurchaseCosts);
}
...
const foodConsumed=p=>foodEvents(p).reduce((a,x)=>a+Number(x.servings||0),0);
const foodFinishEvent=p=>foodEvents(p).find(x=>x.type==='finish');
const foodIsFinished=p=>!!foodFinishEvent(p)||(p.trackingMode==='fixed'&&foodRemaining(p)<=.0001);
const foodRemaining=p=>p.trackingMode==='fixed'?Math.max(0,Number(p.servings)-foodConsumed(p)):null;
const foodCostFor=(p,servings)=>p.trackingMode==='fixed'?Number(p.price)*(Number(servings)||0)/Math.max(Number(p.servings)||1,.01):0;
const foodPurchase=id=>S.foodPurchases.find(x=>x.id===id);
function recalculateFoodPurchaseCosts(p){
  if(!p||p.trackingMode!=='until-finished')return;
  const events=foodEvents(p),portions=events.filter(x=>x.type!=='finish'),finish=events.find(x=>x.type==='finish');
  events.forEach(x=>x.cost=0);
  if(!finish)return;
  if(!portions.length){finish.cost=Number(p.price)||0;return}
  const each=(Number(p.price)||0)/portions.length;
  portions.forEach((x,index)=>x.cost=index===portions.length-1?(Number(p.price)||0)-each*(portions.length-1):each);
}
function foodQuickForm(){return `<div class="card food-entry"><div class="card-head"><div><h3>식품 구매 기록</h3><p class="hint">이 기록은 거래·예산·자산과 완전히 분리된 식비 전용 장부에만 저장됩니다.</p></div></div><div class="food-form food-purchase-form"><label>구매일<input id="foodDate" type="date" value="${iso(today)}"></label><label>식품명<input id="foodTitle" placeholder="예: 닭갈비, 요구르트, 계란"></label><label>구매 가격<input id="foodAmount" inputmode="numeric" placeholder="예: 12000"></label><label>구매 수량<input id="foodQuantity" inputmode="decimal" value="1" placeholder="묶음·개수"></label><label class="food-memo">메모<input id="foodMemo" placeholder="용량, 브랜드 등 선택 입력"></label></div><div class="food-save-row"><span>먹을 때마다 ‘1회 먹음’을 기록하고, 마지막에 ‘다 먹음’으로 완료하세요.</span><button class="btn primary" id="saveFoodEntry">+ 구매 기록 저장</button></div></div>`}
function foodTabs(){return `<div class="tabs food-tabs">${[['inventory','재고·먹기'],['history','먹은 기록'],['stats','통계'],['prices','가격 기록']].map(([id,label])=>`<button data-food-tab="${id}" class="${foodTab===id?'active':''}">${label}</button>`).join('')}</div>`}
function foodInventory(){
  const list=[...S.foodPurchases].sort((a,b)=>Number(foodIsFinished(a))-Number(foodIsFinished(b))||b.purchaseDate.localeCompare(a.purchaseDate));
  return `<div class="card section-gap"><div class="card-head"><div><h3>보유 식품과 먹기 기록</h3><p class="hint">먹은 날짜를 한 번씩 기록하고 마지막에 ‘다 먹음’을 누르면 구매 가격이 실제 섭취 횟수에 맞춰 계산됩니다.</p></div><b>${list.filter(x=>!foodIsFinished(x)).length}개 보유</b></div>${list.length?`<div class="food-inventory-grid">${list.map(p=>{const used=foodConsumed(p),remaining=foodRemaining(p),finished=foodIsFinished(p),fixed=p.trackingMode==='fixed',pct=finished?100:fixed?Math.min(100,used/p.servings*100):0;return `<article class="food-purchase-card ${finished?'finished':''}"><div class="food-purchase-head"><div><span>${esc(p.purchaseDate)}</span><h4>${esc(p.name)}</h4></div><strong>${money(p.price)}</strong></div><div class="food-purchase-meta"><span>구매 ${p.quantity}개</span><span>${used.toFixed(2).replace(/\.00$/,'')}회 먹음</span>${finished&&used?`<span>회당 ${money(p.price/used)}</span>`:''}</div><div class="food-progress"><i style="width:${pct}%"></i></div><div class="food-remain"><b>${finished?'다 먹음':fixed?`${remaining.toFixed(2).replace(/\.00$/,'')}회분 남음`:`${used.toFixed(2).replace(/\.00$/,'')}회 먹은 상태`}</b><span>${finished?'섭취 완료':'날짜별로 계속 기록할 수 있습니다.'}</span></div>${!finished?`<div class="food-consume-actions"><button data-food-consume="${p.id}">1회 먹음</button><button class="primary-mini" data-food-finish="${p.id}">다 먹음</button></div>`:''}<div class="food-card-foot"><span>${esc(p.memo)||'메모 없음'}</span><button data-food-edit="${p.id}">수정</button><button class="danger-text" data-food-delete="${p.id}">삭제</button></div></article>`}).join('')}</div>`:empty('위에서 산 음식과 가격을 먼저 기록해 보세요.')}</div>`
}
function foodHistory(){
  const list=S.foodConsumptions.filter(x=>x.date.startsWith(selectedMonth)).map(x=>({...x,purchase:foodPurchase(x.purchaseId)})).filter(x=>x.purchase).sort((a,b)=>b.date.localeCompare(a.date)||String(b.createdAt).localeCompare(String(a.createdAt)));
  return `<div class="card section-gap"><div class="card-head"><div><h3>먹은 기록</h3><p class="hint">각 기록의 날짜와 메모를 수정할 수 있습니다. ‘다 먹음’을 삭제하면 다시 먹는 중으로 돌아갑니다.</p></div><b>${list.length}건</b></div>${list.length?`<div class="table-wrap"><table class="table responsive-table food-ledger"><thead><tr><th>날짜</th><th>식품</th><th>기록</th><th>계산 식비</th><th>메모</th><th></th></tr></thead><tbody>${list.map(x=>`<tr><td data-label="날짜">${x.date}</td><td data-label="식품"><b>${esc(x.purchase.name)}</b></td><td data-label="기록">${x.type==='finish'?'다 먹음':`${x.servings.toFixed(2).replace(/\.00$/,'')}회 먹음`}</td><td data-label="계산 식비" class="red"><b>${x.purchase.trackingMode==='until-finished'&&!foodIsFinished(x.purchase)?'정산 전':money(x.cost)}</b></td><td data-label="메모">${esc(x.memo)||'-'}</td><td data-label="관리"><button data-food-consumption-edit="${x.id}">수정</button><button data-food-consumption-delete="${x.id}">삭제</button></td></tr>`).join('')}</tbody></table></div>`:empty('선택한 달에 먹은 기록이 없습니다.')}</div>`
}
function foodItemStats(){
  const list=S.foodConsumptions.filter(x=>x.date.startsWith(selectedMonth)),map={};
  list.forEach(x=>{const p=foodPurchase(x.purchaseId);if(!p||x.type==='finish'&&!(Number(x.cost)>0))return;const key=p.name.trim().toLocaleLowerCase('ko'),r=map[key]||(map[key]={name:p.name,cost:0,servings:0,count:0,last:''});r.cost+=Number(x.cost)||foodCostFor(p,x.servings);r.servings+=Number(x.servings)||(x.type==='finish'?1:0);r.count++;if(x.date>r.last)r.last=x.date});
  return Object.values(map).sort((a,b)=>b.cost-a.cost);
}
function foodTrend(){
  const month=S.foodConsumptions.filter(x=>x.date.startsWith(selectedMonth));
  ...
  const out=[];for(let n=5;n>=0;n--){const [y,m]=selectedMonth.split('-').map(Number),d=new Date(y,m-1-n,1),key=monthKey(d);out.push({label:`${d.getMonth()+1}월`,value:S.foodConsumptions.filter(x=>x.date.startsWith(key)).reduce((a,x)=>a+(Number(x.cost)||0),0)})}return out
}
function foodStats(){
  const list=S.foodConsumptions.filter(x=>x.date.startsWith(selectedMonth)),total=list.reduce((a,x)=>a+(Number(x.cost)||0),0),[y,m]=selectedMonth.split('-').map(Number),days=new Date(y,m,0).getDate(),stats=foodItemStats();
  return `<div class="grid cols4 section-gap">${metric('🍴','soft-red','이번 달 먹은 식비',money(total),`${list.length}회 기록`)}${metric('日','soft-green','1일 평균',money(total/days),`${days}일 기준`)}${metric('週','soft-blue','주 평균',money(total/(days/7)),'먹은 비용 기준')}${metric('▦','soft-orange','먹은 식품',`${stats.length}종`,'거래내역과 별도')}</div><div class="card section-gap"><div class="card-head"><div><h3>항목별 소비 통계</h3><p class="hint">구매 가격을 먹은 분량만큼 나누어 계산합니다.</p></div></div>${stats.length?`<div class="table-wrap"><table class="table food-stats-table"><thead><tr><th>식품</th><th>먹은 횟수</th><th>먹은 분량</th><th>월 식비</th><th>회당 평균</th><th>최근 섭취</th></tr></thead><tbody>${stats.map(x=>`<tr><td><b>${esc(x.name)}</b></td><td>${x.count}회</td><td>${x.servings.toFixed(2).replace(/\.00$/,'')}회분</td><td class="red"><b>${money(x.cost)}</b></td><td>${money(x.cost/x.count)}</td><td>${x.last}</td></tr>`).join('')}</tbody></table></div>`:empty('먹은 기록을 추가하면 식품별 통계가 표시됩니다.')}</div><div class="card section-gap"><div class="card-head"><h3>먹은 식비 추이</h3><div class="tabs compact">${['일','주','월'].map(x=>`<button data-food-mode="${x}" class="${foodMode===x?'active':''}">${x}</button>`).join('')}</div></div>${barChart(foodTrend(),'#ff585d')}</div>`
}
function foodPrices(){
  const groups={};S.foodPurchases.forEach(p=>{const key=p.name.trim().toLocaleLowerCase('ko'),g=groups[key]||(groups[key]={name:p.name,rows:[]});g.rows.push({...p,unitPrice:p.price/Math.max(p.quantity,.01)})});
  const list=Object.values(groups).sort((a,b)=>a.name.localeCompare(b.name,'ko'));
  return `<div class="card section-gap"><div class="card-head"><div><h3>구매 가격 기록</h3><p class="hint">같은 식품을 언제 얼마에 샀는지 수량당 가격으로 비교합니다.</p></div></div>${list.length?`<div class="food-price-groups">${list.map(g=>{const rows=g.rows.sort((a,b)=>a.unitPrice-b.unitPrice),best=rows[0];return `<section class="food-price-group"><div><h4>${esc(g.name)}</h4><span>최저 ${money(best.unitPrice)} / 1개</span></div><div class="table-wrap"><table class="table"><thead><tr><th>구매일</th><th>총 가격</th><th>수량</th><th>개당 가격</th></tr></thead><tbody>${rows.map((x,i)=>`<tr class="${i===0?'best-price':''}"><td>${x.purchaseDate}${i===0?' <span class="tag soft-green">최저가</span>':''}</td><td>${money(x.price)}</td><td>${x.quantity}</td><td><b>${money(x.unitPrice)}</b></td></tr>`).join('')}</tbody></table></div></section>`}).join('')}</div>`:empty('구매 기록을 추가하면 식품별 가격을 비교할 수 있습니다.')}</div>`
}
function food(){const monthPurchases=S.foodPurchases.filter(x=>x.purchaseDate.startsWith(selectedMonth)),bought=monthPurchases.reduce((a,x)=>a+x.price,0),consumed=S.foodConsumptions.filter(x=>x.date.startsWith(selectedMonth)).reduce((a,x)=>a+(Number(x.cost)||0),0);return header('식비 관리')+foodQuickForm()+`<div class="food-independent-note">거래·예산·자산과 연결되지 않는 독립 공간입니다. 이번 달 구매 ${money(bought)} · 먹은 비용 ${money(consumed)}</div>`+foodTabs()+(foodTab==='history'?foodHistory():foodTab==='stats'?foodStats():foodTab==='prices'?foodPrices():foodInventory())}
function saveFoodEntry(){const name=$('#foodTitle').value.trim(),price=Math.abs(num($('#foodAmount').value)),quantity=Math.max(.01,decimalNum($('#foodQuantity').value));if(!name||!price)return toast('식품명과 구매 가격을 입력하세요.');const purchaseDate=$('#foodDate').value||iso(today);S.foodPurchases.push({id:uid(),name,purchaseDate,store:'',price,quantity,trackingMode:'until-finished',servings:null,memo:$('#foodMemo').value.trim(),createdAt:new Date().toISOString()});selectedMonth=purchaseDate.slice(0,7);foodTab='inventory';render();toast(`${name} 구매 기록을 저장했습니다.`)}
function consumeFood(id,date=iso(today),memo=''){const p=foodPurchase(id);if(!p||foodIsFinished(p))return toast('이미 다 먹은 식품입니다.');const amount=p.trackingMode==='fixed'?Math.min(1,foodRemaining(p)):1;if(amount<=.0001)return toast('남은 분량이 없습니다.');S.foodConsumptions.push({id:uid(),purchaseId:p.id,type:'portion',date,servings:amount,cost:foodCostFor(p,amount),memo,createdAt:new Date().toISOString()});recalculateFoodPurchaseCosts(p);selectedMonth=date.slice(0,7);render();toast(`${p.name} 1회 먹은 기록을 저장했습니다.`)}
function foodConsumeModal(id){const p=foodPurchase(id);if(!p||foodIsFinished(p))return;$('#submodal').innerHTML=`<div class="modal"><div class="modalbox"><h2>${esc(p.name)} 1회 먹음</h2><div class="modal-grid"><label>먹은 날짜<input id="foodConsumeDate" type="date" value="${iso(today)}"></label><label class="wide">메모<input id="foodConsumeMemo" placeholder="예: 저녁 식사"></label></div><div class="modal-actions"><button class="pill" id="cancelFoodConsume">취소</button><button class="btn primary" id="confirmFoodConsume">기록</button></div></div></div>`;$('#cancelFoodConsume').onclick=closeSubmodal;$('#confirmFoodConsume').onclick=()=>{const date=$('#foodConsumeDate').value,memo=$('#foodConsumeMemo').value.trim();if(!/^\d{4}-\d{2}-\d{2}$/.test(date))return toast('먹은 날짜를 선택하세요.');closeSubmodal();consumeFood(id,date,memo)}}
function finishFoodModal(id){const p=foodPurchase(id);if(!p||foodIsFinished(p))return;$('#submodal').innerHTML=`<div class="modal"><div class="modalbox"><h2>${esc(p.name)} 다 먹음</h2><p class="hint">완료 날짜를 기록하면 지금까지의 1회 먹음 기록에 구매 가격을 나누어 식비를 정산합니다.</p><div class="modal-grid"><label>다 먹은 날짜<input id="foodFinishDate" type="date" value="${iso(today)}"></label><label class="wide">메모<input id="foodFinishMemo" placeholder="예: 마지막으로 먹음"></label></div><div class="modal-actions"><button class="pill" id="cancelFoodFinish">취소</button><button class="btn primary" id="confirmFoodFinish">다 먹음 기록</button></div></div></div>`;$('#cancelFoodFinish').onclick=closeSubmodal;$('#confirmFoodFinish').onclick=()=>{const date=$('#foodFinishDate').value;if(!/^\d{4}-\d{2}-\d{2}$/.test(date))return toast('다 먹은 날짜를 선택하세요.');const servings=p.trackingMode==='fixed'?foodRemaining(p):0;S.foodConsumptions.push({id:uid(),purchaseId:p.id,type:'finish',date,servings,cost:foodCostFor(p,servings),memo:$('#foodFinishMemo').value.trim(),createdAt:new Date().toISOString()});recalculateFoodPurchaseCosts(p);selectedMonth=date.slice(0,7);closeSubmodal();render();toast(`${p.name}을 다 먹은 것으로 기록했습니다.`)}}
function foodConsumptionEditModal(id){const x=S.foodConsumptions.find(v=>v.id===id),p=x&&foodPurchase(x.purchaseId);if(!x||!p)return;$('#submodal').innerHTML=`<div class="modal"><div class="modalbox"><h2>${esc(p.name)} ${x.type==='finish'?'다 먹음':'먹은 기록'} 수정</h2><div class="modal-grid"><label>날짜<input id="editFoodConsumptionDate" type="date" value="${x.date}"></label><label class="wide">메모<input id="editFoodConsumptionMemo" value="${esc(x.memo)}"></label></div><div class="modal-actions"><button class="pill" id="cancelFoodConsumptionEdit">취소</button><button class="btn primary" id="confirmFoodConsumptionEdit">저장</button></div></div></div>`;$('#cancelFoodConsumptionEdit').onclick=closeSubmodal;$('#confirmFoodConsumptionEdit').onclick=()=>{const date=$('#editFoodConsumptionDate').value;if(!/^\d{4}-\d{2}-\d{2}$/.test(date))return toast('날짜를 선택하세요.');x.date=date;x.memo=$('#editFoodConsumptionMemo').value.trim();recalculateFoodPurchaseCosts(p);selectedMonth=date.slice(0,7);closeSubmodal();render();toast('먹은 기록을 수정했습니다.')}}
function deleteFoodConsumption(id){const x=S.foodConsumptions.find(v=>v.id===id),p=x&&foodPurchase(x.purchaseId);if(!x||!p)return;S.foodConsumptions=S.foodConsumptions.filter(v=>v.id!==id);recalculateFoodPurchaseCosts(p);render();toast(x.type==='finish'?'다 먹음 기록을 삭제해 다시 먹는 중으로 돌렸습니다.':'먹은 기록을 삭제했습니다.')}
function foodEditModal(id){const p=foodPurchase(id);if(!p)return;$('#submodal').innerHTML=`<div class="modal"><div class="modalbox"><h2>식품 구매 기록 수정</h2><div class="modal-grid"><label>구매일<input id="editFoodDate" type="date" value="${p.purchaseDate}"></label><label>식품명<input id="editFoodName" value="${esc(p.name)}"></label><label>구매 가격<input id="editFoodPrice" inputmode="numeric" value="${p.price}"></label><label>구매 수량<input id="editFoodQuantity" inputmode="decimal" value="${p.quantity}"></label><label class="wide">메모<input id="editFoodMemo" value="${esc(p.memo)}"></label></div><div class="modal-actions"><button class="pill" id="cancelFoodEdit">취소</button><button class="btn primary" id="confirmFoodEdit">저장</button></div></div></div>`;$('#cancelFoodEdit').onclick=closeSubmodal;$('#confirmFoodEdit').onclick=()=>{const name=$('#editFoodName').value.trim(),price=Math.abs(num($('#editFoodPrice').value));if(!name||!price)return toast('식품명과 가격을 입력하세요.');Object.assign(p,{purchaseDate:$('#editFoodDate').value,name,price,store:'',quantity:Math.max(.01,decimalNum($('#editFoodQuantity').value)),memo:$('#editFoodMemo').value.trim()});recalculateFoodPurchaseCosts(p);closeSubmodal();render();toast('구매 기록을 수정했습니다.')}}

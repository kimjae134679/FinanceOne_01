...
function calendar(){const dates=periodDates(),first=new Date(`${dates[0]}T00:00:00`).getDay(),map={};calendarTx().filter(x=>x.type==='지출').forEach(x=>map[x.date]=(map[x.date]||0)+Math.abs(x.amount));const max=Math.max(...Object.values(map),0);return `<div class="calendar"><div class="calendar-heat-legend"><span>적은 지출</span><i></i><span>많은 지출</span></div><div class="weekdays">${['일','월','화','수','목','금','토'].map(x=>`<b>${x}</b>`).join('')}</div><div class="cal-grid">${'<i></i>'.repeat(first)}${dates.map(date=>{const v=map[date]||0;return `<button data-date="${date}" class="${v?'spent':''}" ${calendarHeatStyle(v,max)} title="${date}"><b>${Number(date.slice(8))}</b>${periodMode==='rolling'?`<small>${Number(date.slice(5,7))}월</small>`:''}${v?`<span>${money(v)}</span>`:''}</button>`}).join('')}</div></div>`}
...
function saveFoodMeal(date,purchaseIds,mealId=''){
  const current=mealId?S.foodConsumptions.filter(x=>foodMealKey(x)===mealId):[],currentByPurchase=new Map(current.map(x=>[x.purchaseId,x])),id=mealId||`food-meal-${uid()}`,createdAt=current[0]?.createdAt||new Date().toISOString(),affected=new Set([...current.map(x=>x.purchaseId),...purchaseIds]);
  S.foodConsumptions=S.foodConsumptions.filter(x=>foodMealKey(x)!==id);
  purchaseIds.forEach(purchaseId=>{const old=currentByPurchase.get(purchaseId),p=foodPurchase(purchaseId),pack=p?foodPackageMeasure(p):{unit:'회',dimension:'event'};S.foodConsumptions.push(old?{...old,mealId:id,date}:{id:uid(),mealId:id,purchaseId,type:'portion',date,servings:1,amount:1,unit:pack.dimension==='count'?pack.unit:'회',cost:0,memo:'',createdAt})});
...
function foodMealModal(mealId=''){
  ...
  $('#submodal').innerHTML=`<div class="modal"><div class="modalbox food-meal-box"><h2>${mealId?'먹은 기록 수정':'먹은 기록'}</h2><label class="food-meal-date">먹은 날짜<input id="foodMealDate" type="date" value="${date}"></label><div class="food-meal-list">${choices.map(p=>`<label class="food-meal-option ${selected.has(p.id)?'selected':''}"><input type="checkbox" data-food-meal-pick value="${p.id}" ${selected.has(p.id)?'checked':''}><span><b>${esc(foodPurchaseTitle(p))}</b><small>${esc(p.store||(p.source==='existing'?'기존 보유 재고':'구매처 미입력'))} · ${foodConsumed(p)}회 먹음${foodIsFinished(p)?' · 다 먹음':''}</small></span><em>1회</em></label>`).join('')}</div><div class="modal-actions"><button class="pill" id="cancelFoodMeal">취소</button><button class="btn primary" id="confirmFoodMeal">${mealId?'저장':'기록'}</button></div></div></div>`;
  ...
  $('#cancelFoodMeal').onclick=closeSubmodal;$('#confirmFoodMeal').onclick=()=>{const chosen=$$('[data-food-meal-pick]:checked').map(x=>x.value),chosenDate=$('#foodMealDate').value;if(!/^\d{4}-\d{2}-\d{2}$/.test(chosenDate))return toast('먹은 날짜를 선택하세요.');if(!chosen.length)return toast('먹은 식재료를 하나 이상 선택하세요.');saveFoodMeal(chosenDate,chosen,mealId)};
}
